(define
  ...
  (:types horizon-value - object aircraft person city flevel - object)
   (:constants horzn1 horzn10 horzn25 - horizon-value) (:predicates (horizon ?h - horizon-value) (horizon-decrement ?h0 ?h1 ?h2 - horizon-value)
  ...
  (:action complete-flying
    ...
    :precondition (and (horizon ?hcur) (horizon-decrement ?hcur horzn10 ?hnew) (flying ?a ?c2) (fuel-level ?a ?l1) (next ?l2 ?l1))
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 10) (probabilistic 1/25 (and (not (flying ?a ?c2)) (at-aircraft ?a ?c2) (not (fuel-level ?a ?l1)) (fuel-level ?a ?l2))))
  )
  ...
  (:action complete-zooming
    :parameters (?hcur ?hnew - horizon-value ?a - aircraft ?c2 - city ?l1 ?l2 - flevel)
    :precondition (and (horizon ?hcur) (horizon-decrement ?hcur horzn25 ?hnew) (zooming ?a ?c2) (fuel-level ?a ?l1) (next ?l2 ?l1))
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 25) (probabilistic 1/15 (and (not (zooming ?a ?c2)) (at-aircraft ?a ?c2) (not (fuel-level ?a ?l1)) (fuel-level ?a ?l2))))
  )
