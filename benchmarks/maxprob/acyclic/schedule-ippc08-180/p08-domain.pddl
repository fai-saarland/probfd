(define
  ...
  (:constants horzn1 - horizon-value  C0 C1 C2 - class Arrivals-and-updating Cleanup-and-service - phase U0 U1 U2 U3 U4 U5 - timestep P0 P1 P2 P3 P4 P5 P6 P7 P8 P9 P10 P11 P12 P13 P14 - packet)
  ...
  (:action process-arrivals-C0
    ...
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 1) (not (need-to-process-arrivals C0)) (probabilistic 1/100 (and (not (available ?arriving-packet)) (time-to-live ?arriving-packet ?t) (packet-class ?arriving-packet C0))))
  )
  (:action process-arrivals
    ...
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 1) (not (need-to-process-arrivals ?c)) (probabilistic 11/20 (and (not (available ?arriving-packet)) (time-to-live ?arriving-packet ?t) (packet-class ?arriving-packet ?c))))
  )
  (:action time-update
    :parameters (?hcur ?hnew - horizon-value ?t0 ?t1 - timestep)
    :precondition (and (horizon ?hcur) (horizon-decrement ?hcur horzn1 ?hnew) (current-phase Arrivals-and-updating) (current-timestep ?t0) (next-timestep ?t0 ?t1) (not (need-to-process-arrivals C0)) (not (need-to-process-arrivals C1)) (not (need-to-process-arrivals C2)))
  ...
  (:action reclaim-packet-C1
    ...
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 1) (not (packet-class ?p C1)) (not (time-to-live ?p ?t)) (available ?p) (probabilistic 1/100 (and (not (alive)))))
  )
  (:action reclaim-packet-C2
    ...
    :effect (and (not (horizon ?hcur)) (horizon ?hnew) (increase (total-cost) 1) (not (packet-class ?p C2)) (not (time-to-live ?p ?t)) (available ?p) (probabilistic 1/200 (and (not (alive)))))
  )
  (:action packet-serve
