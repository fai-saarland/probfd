...
public:
    ...
    template <typename T>
    T get(const Context& context, const std::string& key) const
    {
    ...
    template <typename T>
    std::vector<T>
    get_list(const Context& context, const std::string& key) const
    {
        return get<std::vector<T>>(context, key);
    }
...
void verify_list_non_empty(
    ...
    const std::string& key)
{
    if (std::vector<T> list = opts.get_list<T>(context, key); list.empty()) {
