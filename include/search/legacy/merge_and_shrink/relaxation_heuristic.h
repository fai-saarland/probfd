#ifndef LEGACY_MERGE_AND_SHRINK_RELAXATION_HEURISTIC_H
#define LEGACY_MERGE_AND_SHRINK_RELAXATION_HEURISTIC_H
...
struct UnaryOperator {
    ...
    UnaryOperator(
        const std::vector<Proposition*>& pre,
        Proposition* eff,
        int operator_no_,
        int base)
...
struct Proposition {
    ...
    std::vector<UnaryOperator*>
        effect_of; // Michael: for bisimulation - temporary change
    std::set<UnaryOperator*>
        marked_ops; // Michael: for bisimulation - temporary change
