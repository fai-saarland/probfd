#ifndef LEGACY_MERGE_AND_SHRINK_SCC_H
#define LEGACY_MERGE_AND_SHRINK_SCC_H
...
/*
  ...
  Instantiate class SCC with a graph represented as a std::vector of
  std::vectors, where graph[i] is the std::vector of successors of vertex i.
...
public:
    ...
    const std::vector<std::vector<int>>& get_sccs() const { return sccs; }

    const std::vector<int>& get_vars_scc() const { return vars_scc; }
    const std::vector<int>& get_scc_layer() const { return scc_layer; }
    const std::vector<int>& get_vars_layer() const { return vars_layer; }

    const std::vector<std::set<int>> get_scc_graph() const { return scc_graph; }
};
