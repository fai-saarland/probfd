#ifndef LEGACY_MERGE_AND_SHRINK_MERGE_AND_SHRINK_HEURISTIC_H
#define LEGACY_MERGE_AND_SHRINK_MERGE_AND_SHRINK_HEURISTIC_H
...
class MergeAndShrinkHeuristic : public Heuristic {
    ...
    const int merge_size_limit; // Avoid merging a variable if the result will
                                // pass this size
    const int
        max_branching_merge; // Maximum branching factor for multiple merge
    ...
    // new methods for DFS
    void
    prepare_after_merge(Abstraction* abstraction, bool force_label_reduction)
        const;
    void prepare_before_merge(
        Abstraction* abstraction,
        Abstraction* other_abstraction) const;
    void build_atomic_abstractions();
    void
    build_multiple_abstractions(std::vector<Abstraction*>& res_abstractions);
    bool build_abstraction_dfs(
        Abstraction* abstraction,
...
public:
    std::vector<Abstraction*>& get_abstractions() { return abstractions; }
