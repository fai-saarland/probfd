#ifndef LEGACY_MERGE_AND_SHRINK_FF_HEURISTIC_H
#define LEGACY_MERGE_AND_SHRINK_FF_HEURISTIC_H
...
class FFHeuristic : public AdditiveHeuristic {
    ...
    void mark_relaxed_plan(
        const GlobalState& state,
        Proposition* goal); // Peter: Needed for M&S for finding label sets
protected:
    virtual void initialize();
    virtual int compute_heuristic(const GlobalState& state);
    void compute_relaxed_plan(
        const GlobalState& state,
        RelaxedPlan& returned_relaxed_plan); // Peter: Needed for M&S for
                                             // finding label set
public:
