#ifndef LEGACY_MERGE_AND_SHRINK_LINEAR_MERGE_STRATEGY_H
#define LEGACY_MERGE_AND_SHRINK_LINEAR_MERGE_STRATEGY_H
...
#include "legacy/merge_and_shrink/scc.h"
...
protected:
    ...
    bool
    filter(std::vector<int>& vars, const std::vector<bool>& criterion) const
    {
        std::vector<int> aux;
...
public:
    ...
    virtual void init() = 0;
    virtual void disable_incremental() { allow_incremental = false; }
    ...
    virtual bool reduce_labels_before_merge() const { return false; }
};
...
public:
    ...
    virtual std::string get_name() const { return "CG"; }
};
...
public:
    ...
    virtual void select_next(int var_no);
    virtual void filter(std::vector<int>& vars, Abstraction* abstraction);
    virtual std::string get_name() const { return "GOAL"; }
};

class MergeCriterionRelevant : public MergeCriterionCG {
    virtual void init();
    virtual std::string get_name() const { return "RELEVANT"; }
};
...
class MergeCriterionMinSCC : public MergeCriterion {
    ...
    void forbid_scc_descendants(
        int scc_index,
        const std::vector<std::set<int>>& scc_graph,
        std::vector<bool>& forbidden_sccs) const;

public:
    MergeCriterionMinSCC(bool reverse_ = false, bool tie_by_level_ = true)
        : reverse(reverse_)
    ...
    virtual void select_next(int var_no);
    virtual void filter(std::vector<int>& vars, Abstraction* abstraction);
    virtual std::string get_name() const { return "SCC"; }
};
...
public:
    ...
    virtual void select_next(int var_no);
    virtual void filter(std::vector<int>& vars, Abstraction* abstraction);
    virtual std::string get_name() const { return "EMPTY"; }
    virtual bool reduce_labels_before_merge() const { return true; }
};
...
public:
    ...
    virtual void select_next(int var_no);
    virtual void filter(std::vector<int>& vars, Abstraction* abstraction);
    virtual std::string get_name() const { return "EMPTYGOAL"; }
    virtual bool reduce_labels_before_merge() const { return true; }
};
...
public:
    ...
    virtual void select_next(int var_no);
    virtual void filter(std::vector<int>& vars, Abstraction* abstraction);
    virtual std::string get_name() const { return "NUM"; }
    virtual bool reduce_labels_before_merge() const { return true; }
};

enum MergeOrder { LEVEL, REVERSE_LEVEL, RANDOM };
...
class MultipleLinearMergeStrategy {
    ...
    void
    order_vars(Abstraction* abstraction, std::vector<int>& selected_vars) const;

public:
    MultipleLinearMergeStrategy(
        const std::vector<std::shared_ptr<MergeCriterion>>& merge_criteria,
        const MergeOrder& merge_order,
        bool is_first);
    ...
    void
    next_vars(std::vector<int>& selected_vars, Abstraction* abstraction = 0)
        const;
};
