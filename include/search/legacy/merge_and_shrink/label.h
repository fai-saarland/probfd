#ifndef LEGACY_MERGE_AND_SHRINK_LABEL_H
#define LEGACY_MERGE_AND_SHRINK_LABEL_H
...
class Label {
    ...
    // the "first" label of all parent labels when constructing a
    // CompositeLabel.
    const std::vector<GlobalCondition>& prevail;
    const std::vector<GlobalEffect>& pre_post;
...
public:
    const std::vector<GlobalCondition>& get_prevail() const { return prevail; }
    const std::vector<GlobalEffect>& get_pre_post() const { return pre_post; }
...
public:
    OperatorLabel(
        int id,
        int cost,
        const std::vector<GlobalCondition>& precond,
        const std::vector<GlobalEffect>& pre_post);
...
public:
    ...
    const std::vector<Label*>& get_parents() const { return parents; }
};
