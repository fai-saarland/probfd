...
/*
  ...
  task implementations (subclasses of ProbabilisticTask). Each proxy class
  knows which ProbabilisticTask it belongs to and uses its methods to retrieve
  information about the task. RootTask is the ProbabilisticTask that
  ...
  Since proxy classes only store a reference to the ProbabilisticTask and
...
public:
    FactProxy(const ProbabilisticTask& task, int var_id, int value);
    FactProxy(const ProbabilisticTask& task, const FactPair& fact);
    ~FactProxy() = default;
...
public:
    FactsProxyIterator(const ProbabilisticTask& task, int var_id, int value)
        : task(&task)
        , var_id(var_id)
        , value(value)
    {
...
public:
    explicit FactsProxy(const ProbabilisticTask& task)
        : task(&task)
    {
    }
    ~FactsProxy() = default;
...
public:
    VariableProxy(const ProbabilisticTask& task, int id)
        : task(&task)
        , id(id)
    {
    }
...
public:
    ...
    explicit VariablesProxy(const ProbabilisticTask& task)
        : task(&task)
    {
    }
...
public:
    ...
    EffectProxy operator[](std::size_t eff_index) const
    {
        ...
        return EffectProxy(*task, op_index, outcome_index, eff_index, is_axiom);
    }
};
...
public:
    ...
    ~OutcomeProxy() = default;

    EffectsProxy get_effects() const
    {
        return EffectsProxy(*task, op_index, outcome_index, is_axiom);
    }
...
public:
    using ItemType = OutcomeProxy;
    OutcomesProxy(const ProbabilisticTask& task, int op_index, bool is_axiom)
        : task(&task)
        , op_index(op_index)
        , is_axiom(is_axiom)
    {
    }
    ~OutcomesProxy() = default;
    ...
    OutcomeProxy operator[](std::size_t eff_index) const
    {
        assert(eff_index < size());
        return OutcomeProxy(*task, op_index, eff_index, is_axiom);
    }
};
...
public:
    OperatorProxy(const ProbabilisticTask& task, int index, bool is_axiom)
        : task(&task)
        , index(index)
    ...
    OutcomesProxy get_outcomes() const
    {
        return OutcomesProxy(*task, index, is_an_axiom);
    }
...
public:
    ...
    explicit OperatorsProxy(const ProbabilisticTask& task)
        : task(&task)
    {
    }
...
public:
    explicit GoalsProxy(const ProbabilisticTask& task)
        : ConditionsProxy(task)
    {
    }
    ~GoalsProxy() = default;
...
public:
    ...
    State(const ProbabilisticTask& task, std::vector<int>&& values);
...
inline FactProxy::FactProxy(const ProbabilisticTask& task, const FactPair& fact)
    : task(&task)
    , fact(fact)
{
