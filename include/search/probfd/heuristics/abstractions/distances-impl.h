    ...
    const std::vector<StateID>& pruned_states)
{
    ...
    std::deque<AbstractStateIndex> queue({initial_state});
    std::set<AbstractStateIndex> seen({initial_state});

    while (!queue.empty()) {
        AbstractStateIndex s = queue.front();
        queue.pop_front();
    ...
    double max_time)
{
    ...
    class WrapperHeuristic : public AbstractionEvaluator {
        const std::vector<StateID>& pruned_states;
        const AbstractionEvaluator& parent;
    ...
    TATopologicalValueIteration<AbstractStateIndex, Action>
        vi;
