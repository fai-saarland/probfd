...
public:
    ...
    // constructed.
    bool supports_conditional_effects()
    {
        return conditional_effects_supported;
    }

    // ------------------------------------------------------------------------------
    // methods needed by both landmarkgraph-factories and
    // non-landmarkgraph-factories
    inline const std::set<LandmarkNode*>& get_nodes() const { return nodes; }
    ...
    LandmarkGraph(const options::Options& opts);
    ...
    get_operators_including_eff(const std::pair<int, int>& eff) const
    {
    ...
    bool simple_landmark_exists(
        const std::pair<int, int>& lm) const; // not needed by HMLandmark
    bool disj_landmark_exists(const std::set<std::pair<int, int>>& lm)
        const; // not needed by HMLandmark
    bool landmark_exists(
        const std::pair<int, int>& lm) const; // not needed by HMLandmark
    bool exact_same_disj_landmark_exists(
        const std::set<std::pair<int, int>>& lm) const;

    LandmarkNode& landmark_add_simple(const std::pair<int, int>& lm);
    LandmarkNode&
    landmark_add_disjunctive(const std::set<std::pair<int, int>>& lm);
    LandmarkNode&
    landmark_add_conjunctive(const std::set<std::pair<int, int>>& lm);
...
private:
    ...
    int needed_cost;
    int landmarks_cost;
    std::unordered_map<std::pair<int, int>, LandmarkNode*, hash_int_pair>
        simple_lms_to_nodes;
    std::unordered_map<std::pair<int, int>, LandmarkNode*, hash_int_pair>
        disj_lms_to_nodes;
