#ifdef TASK_UTILS_SUCCESSOR_GENERATOR_INTERNALS_H
...
template<typename Entry>
void
GeneratorForkBinary<Entry>::generate_applicable_ops(
...
template<typename Entry>
void
GeneratorForkMulti<Entry>::generate_applicable_ops(
    const GlobalState& state,
    std::vector<Entry>& applicable_ops) const
{
...
template<typename Entry>
void
GeneratorSwitchVector<Entry>::generate_applicable_ops(
    const GlobalState& state,
    std::vector<Entry>& applicable_ops) const
{
...
GeneratorSwitchHash<Entry>::GeneratorSwitchHash(
    int switch_var_id,
    std::unordered_map<int, std::unique_ptr<GeneratorBase<Entry>>>&& generator_for_value)
    : switch_var_id(switch_var_id)
    , generator_for_value(move(generator_for_value))
{
}
...
template<typename Entry>
void
GeneratorSwitchHash<Entry>::generate_applicable_ops(
    const GlobalState& state,
    std::vector<Entry>& applicable_ops) const
{
    int val = state[switch_var_id];
...
template<typename Entry>
void
GeneratorSwitchSingle<Entry>::generate_applicable_ops(
    const GlobalState& state,
    std::vector<Entry>& applicable_ops) const
{
...
template<typename Entry>
void
GeneratorLeafVector<Entry>::generate_applicable_ops(
    const GlobalState&,
    std::vector<Entry>& applicable_ops) const
{
...
template<typename Entry>
void
GeneratorLeafSingle<Entry>::generate_applicable_ops(
    const GlobalState&,
    std::vector<Entry>& applicable_ops) const
{
