...
public:
    ...
    friend bool operator==(const ProxyIterator& lhs, const ProxyIterator& rhs)
    {
        assert(lhs.collection == rhs.collection);
        return lhs.pos == rhs.pos;
    }

    friend auto operator<=>(const ProxyIterator& lhs, const ProxyIterator& rhs)
    {
        assert(lhs.collection == rhs.collection);
        return lhs.pos <=> rhs.pos;
    }
...
struct ProxyCollectionTag {};

template <typename T>
concept ProxyCollectionEnabled = std::derived_from<T, ProxyCollectionTag>;

template <SizedSubscriptable T>
    requires ProxyCollectionEnabled<T>
auto begin(const T& t)
{
    return ProxyIterator(*static_cast<const T*>(&t), 0);
}
...
auto end(const T& t)
{
    return ProxyIterator(*static_cast<const T*>(&t), t.size());
}
