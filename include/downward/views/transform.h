#ifndef DOWNWARD_TRANSFORM_VIEW_H
#define DOWNWARD_TRANSFORM_VIEW_H
...
template <std::ranges::input_range Vw, auto F>
    requires std::ranges::view<Vw> && std::regular_invocable<
                                          decltype(F),
...
private:
    ...
    public:
        ...
        constexpr decltype(auto) operator*() const
            noexcept(noexcept(std::invoke(F, *Current)))
        {
        ...
        constexpr decltype(auto) operator[](const difference_type Idx) const
            noexcept(noexcept(std::invoke(F, Current[Idx])))
            requires std::ranges::random_access_range<Base>
        {
            return std::invoke(F, Current[Idx]);
        }
...
public:
    ...
    constexpr explicit transform_view(Vw Range_) noexcept(
