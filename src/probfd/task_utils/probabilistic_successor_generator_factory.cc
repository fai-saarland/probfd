...
static GeneratorPtr construct_fork(vector<GeneratorPtr> nodes)
{
    ...
    if (size == 1) {
        ...
        return std::make_unique<ProbabilisticGeneratorForkBinary>(
            std::move(nodes.at(0)),
            std::move(nodes.at(1)));
    } else {
        /* This general case includes the case size == 0, which can
           (only) happen for the root for tasks with no operators. */
        return std::make_unique<ProbabilisticGeneratorForkMulti>(
            std::move(nodes));
    }
}

GeneratorPtr ProbabilisticSuccessorGeneratorFactory::construct_leaf(
    OperatorRange range) const
{
    ...
    if (operators.size() == 1) {
        return std::make_unique<ProbabilisticGeneratorLeafSingle>(
            operators.front());
    } else {
        return std::make_unique<ProbabilisticGeneratorLeafVector>(
            std::move(operators));
    }
}

GeneratorPtr ProbabilisticSuccessorGeneratorFactory::construct_switch(
    int switch_var_id,
    ValuesAndGenerators values_and_generators) const
{
    ...
    if (num_children == 1) {
        ...
        return std::make_unique<ProbabilisticGeneratorSwitchSingle>(
            switch_var_id,
            value,
            std::move(generator));
    }
