...
void FTSFactory::build_state_data(VariableProxy var)
{
    ...
    const int range = task_proxy.get_variables()[var_id].get_domain_size();
    ts_data.num_states = range;

    int goal_value = -1;
    for (const GoalsProxy goals = task_proxy.get_goals();
         FactProxy goal : goals) {
...
void FTSFactory::build_transitions_for_operator(ProbabilisticOperatorProxy op)
{
    ...
    */
    for (FactProxy precondition : op.get_preconditions()) {
        if (const int var_no = precondition.get_variable().get_id();
            !has_effect_on_var[var_no]) {
    ...
    for (int var_no = 0; var_no < num_variables; ++var_no) {
        ...
        /*
          TODO: Our method for generating transitions is only guaranteed
    ...
    const Labels& labels)
{
    ...
    if (!irrelevant_labels.empty()) {
        ...
        for (int state = 0; state < num_states; ++state) {
            // FIXME wrong arity
            transitions.emplace_back(state, std::vector(1, state));
        }

        const int new_local_label = ts_data.local_label_infos.size();
        for (const int label : irrelevant_labels) {
