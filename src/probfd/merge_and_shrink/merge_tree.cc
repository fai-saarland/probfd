    ...
    utils::LogProxy& log) const
{
    ...
    for (int i = 0; i < current_indentation; ++i) { log.print(" "); }
...
pair<int, int> MergeTree::get_next_merge(int new_index) const
{
    ...
    int new_index) const
{
    ...
    while (!copy->is_leaf()) {
        ...
        if (left == ts_index1 || right == ts_index1) { ++found_indices; }

        if (left == ts_index2 || right == ts_index2) {
...
void MergeTree::update(const pair<int, int>& merge, int new_index)
{
    const int ts_index1 = merge.first;
    const int ts_index2 = merge.second;
    ...
    if (first_parent == second_parent) { // given merge already in the tree
        ...
        const MergeTreeNode* surviving_node = nullptr;
