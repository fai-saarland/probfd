...
LandmarkGraphMerged::LandmarkGraphMerged(const options::Options &opts)
    : LandmarkFactory(opts),
      lm_graphs(opts.get_list<std::shared_ptr<LandmarkGraph> >("lm_graphs")) {
}
...
static std::shared_ptr<LandmarkGraph> _parse(options::OptionParser &parser) {
    ...
    parser.add_list_option<std::shared_ptr<LandmarkGraph> >("lm_graphs");
    LandmarkGraph::add_options_to_parser(parser);
    options::Options opts = parser.parse();

    opts.verify_list_non_empty<std::shared_ptr<LandmarkGraph> >("lm_graphs");
    ...
    if (parser.dry_run()) {
        ...
        vector<std::shared_ptr<LandmarkGraph> > lm_graphs = opts.get_list<std::shared_ptr<LandmarkGraph> >("lm_graphs");
        ...
        std::shared_ptr<LandmarkGraph> graph = std::shared_ptr<LandmarkGraph>(lm_graph_factory.compute_lm_graph());
        return graph;
    }
}
