...
LandmarkCostAssignment::LandmarkCostAssignment(
    std::shared_ptr<LandmarkGraph> graph, OperatorCost cost_type_)
...
LandmarkUniformSharedCostAssignment::LandmarkUniformSharedCostAssignment(
    std::shared_ptr<LandmarkGraph> graph, bool use_action_landmarks_, OperatorCost cost_type_)
...
LandmarkEfficientOptimalSharedCostAssignment::LandmarkEfficientOptimalSharedCostAssignment(
    std::shared_ptr<LandmarkGraph> graph, OperatorCost cost_type, lp::LPSolverType solver_type)
    ...
    , lp_solver(solver_type)
{
    ...
    for (size_t op_id = 0; op_id < g_operators.size(); ++op_id) {
        ...
        lp_constraints[op_id].set_upper_bound(get_adjusted_action_cost(g_operators[op_id], cost_type));
    }
#endif
}
