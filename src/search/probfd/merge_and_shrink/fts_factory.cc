        ...
        for (int e = 0; e != num_outcomes; ++e) {
            ...
            if (eff_id != num_effs &&
                outcome_effects[eff_id].get_fact().get_variable().get_id() ==
                    var_id) {
                // Has an effect on this variable
                t.targets[e] = outcome_effects[eff_id].get_fact().get_value();
                ++eff_id;
            } else {
                // Has no effect on this variable
                t.targets[e] = pre_val;
...
void FTSFactory::build_transitions(const Labels& labels)
{
    ...
    */
    for (TransitionSystemData& ts_data : transition_system_data_by_var) {
        ...
        auto cmp = [](LocalLabelInfo* left, LocalLabelInfo* right) {
        ...
        for (std::size_t index = 0; index != local_label_infos.size();
             ++index) {
            auto& local_label_info = local_label_infos[index];
