...
static void normalize_given_transitions(vector<Transition>& transitions)
{
    ...
    transitions.erase(
        unique(transitions.begin(), transitions.end()),
        transitions.end());
}
...
TransitionSystem::TransitionSystem(const TransitionSystem& other)
    ...
    , label_equivalence_relation(std::make_unique<LabelEquivalenceRelation>(
    ...
    utils::LogProxy& log)
{
    ...
    assert(
        ts1.are_transitions_sorted_unique() &&
        ts2.are_transitions_sorted_unique());
    ...
    */
    ...
    for (GroupAndTransitions gat : ts1) {
        ...
        for (int label_no : group1) {
            int group2_id =
                ts2.label_equivalence_relation->get_group_id(label_no);
    ...
    */
    ...
    unique_ptr<LabelEquivalenceRelation> label_equivalence_relation =
        std::make_unique<LabelEquivalenceRelation>(labels, label_groups);
    ...
    for (int group_id1 = 0; group_id1 < label_equivalence_relation->get_size();
        ...
        if (!label_equivalence_relation->is_empty_group(group_id1)) {
            const vector<Transition>& transitions1 =
                transitions_by_group_id[group_id1];
            for (int group_id2 = group_id1 + 1;
                 group_id2 < label_equivalence_relation->get_size();
                 ++group_id2) {
                if (!label_equivalence_relation->is_empty_group(group_id2)) {
                    vector<Transition>& transitions2 =
                        transitions_by_group_id[group_id2];
    ...
    bool only_equivalent_labels)
{
    ...
    */
    ...
    if (only_equivalent_labels) {
        ...
        */
        ...
        for (const pair<int, vector<int>>& mapping : label_mapping) {
            ...
            for (int old_label_no : old_label_nos) {
                int group_id =
                    label_equivalence_relation->get_group_id(old_label_no);
                if (seen_group_ids.insert(group_id).second) {
                    affected_group_ids.insert(group_id);
                    const vector<Transition>& transitions =
                        transitions_by_group_id[group_id];
        ...
        */
        label_equivalence_relation->apply_label_mapping(
            label_mapping,
            &affected_group_ids);
        ...
        */
        for (size_t i = 0; i < label_mapping.size(); ++i) {
            ...
            assert(
                label_equivalence_relation->get_group_id(
                    label_mapping[i].first) ==
...
bool TransitionSystem::are_transitions_sorted_unique() const
{
    for (GroupAndTransitions gat : *this) {
        if (!utils::is_sorted_unique(gat.transitions)) return false;
    }
    return true;
}
    ...
    if (log.is_at_least_debug()) {
        ...
        for (GroupAndTransitions gat : *this) {
            ...
            for (LabelConstIter label_it = label_group.begin();
                 label_it != label_group.end();
                 ++label_it) {
                if (label_it != label_group.begin()) log << ",";
                log << *label_it;
            }
            log << endl;
            log << "transitions: ";
