...
void MergeAndShrinkHeuristic::dump_options() const
{
    ...
    case MERGE_LINEAR_MULTIPLE_DFS:
        cout << "Multiple:";
        break; // TODO correct?
    case MERGE_LINEAR:
    ...
    cout << "Label reduction: "
         ...
         << (prune_unreachable ? "yes" : "no") << endl
         << "Prune irrelevant states: "
         << (prune_irrelevant ? "yes" : "no") << endl
...
void MergeAndShrinkHeuristic::warn_on_unusual_options() const
{
    ...
    if (empty_label_shrink_strategy) {
        ...
        if (!check_unsolvability) {
            string dashes(79, '=');
            cerr << dashes << endl
                 << ("WARNING! Empty label shrinking is activated and the check for "
...
void MergeAndShrinkHeuristic::get_candidate_vars(vector<int> &res,
        ...
        Abstraction *abstraction) const
{
    ...
    for (size_t i = 0; i < g_variable_domain.size(); ++i) {
        ...
        //c) Skip merging variables whose size is surpassed
        if (abstraction
                && abstraction->size()*atomic_abstractions[i]->size() > merge_size_limit) {
...
void MergeAndShrinkHeuristic::prepare_after_merge(Abstraction *abstraction,
        bool force_reduce_labels) const
{
    ...
    if (shrink_strategy->reduce_labels_before_shrinking() || force_reduce_labels
        ...
        // Note: label inheritance for atomic abstractions works only for linear merging strategies,
        // as the label set cannot be empty for the MAIN abstraction
        //     other_abstraction->normalize(false, shrink_strategy->get_caught_labels_set(),
        // 				 use_label_inheritance, new_approach);
    }

    if (empty_label_shrink_strategy) {
        cout << "applying empty label shrinking" << endl;
        //HACK, we need two abstractions for shrink_before_merge, so instead we call shrink_atomic (it does the same in the empty label shrinking)
        //empty_label_shrink_strategy->shrink_before_merge(*abstraction, *other_abstraction);
        ...
        if (shrink_strategy->reduce_labels_before_shrinking()
            ...
            abstraction->normalize(
                false/*use_label_reduction*/ /*no need to reduce labels again -- transitions with same labels are already merged!*/,
                shrink_strategy->get_caught_labels_set(),
                use_label_inheritance, new_approach);
            //other_abstraction->normalize(false, shrink_strategy->get_caught_labels_set(), use_label_inheritance, new_approach);
        } else {
    ...
    abstraction->compute_distances(prune_unreachable, prune_irrelevant,
                                   continue_if_unsolvable, use_uniform_distances);
    // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
    //abstraction->statistics(use_expensive_statistics);
    ...
    ostringstream out;
    out << "abstract.space" << "." << abs_count << ".dot";
    ofstream myfile;
...
void MergeAndShrinkHeuristic::prepare_before_merge(Abstraction *abstraction,
        Abstraction *other_abstraction) const
{
    ...
    other_abstraction->compute_distances(prune_unreachable, prune_irrelevant,
                                         continue_if_unsolvable, use_uniform_distances);
    // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
    //other_abstraction->statistics(use_expensive_statistics);
#ifdef DEBUG_ABSTRACTIONS
    ...
    */
    ...
    ostringstream out2;
    out2 << "abstract.space.after.shrink" << "." << abs_count++ << ".dot";
    ofstream myfile2;
    ...
    abstraction->normalize(use_label_reduction,
                           shrink_strategy->get_caught_labels_set(), false);
    // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
    //abstraction->statistics(use_expensive_statistics);
    ...
    vector<Abstraction *>  &res_abstractions)
{
    ...
    for (size_t i = 0; i < atomic_abstractions.size(); ++i) {
        if (!atomic_abstractions[i]->is_solvable() && !continue_if_unsolvable) {
            cout << "Proved unsolvability by: " << *(atomic_abstractions[i]) << endl;
    ...
    // Generate the instance of multiple linear merge strategy to select variables
    MultipleLinearMergeStrategy order(merge_criteria, merge_order,
                                      true /*is_first*/);
    ...
    for (size_t i = 0; i < selected_variables.size()
            && i <= (size_t) max_branching_merge; i++) {
        ...
        if (build_abstraction_dfs(abstraction, order, explored_varsets,
                                  res_abstractions)) {
            return;
        }
    }
...
bool MergeAndShrinkHeuristic::build_abstraction_dfs(
    Abstraction *abstraction, MultipleLinearMergeStrategy &order,
    set<set<int> > &explored_varsets, vector<Abstraction *> &res_abstractions) const
{
    ...
    if (selected_variables.empty()) {
        ...
        for (size_t i = 0; i < selected_variables.size(); i++) {
            ...
            if (prune_unreachable || prune_irrelevant
                ...
                new_abstraction->compute_distances(prune_unreachable, prune_irrelevant,
                                                   continue_if_unsolvable, use_uniform_distances);
            ...
            prepare_after_merge(new_abstraction, order.reduce_labels_before_merge());
            ...
            bool finished = false;
            if (!new_abstraction->is_solvable() && !continue_if_unsolvable) {
                cout << "Proved unsolvability by new_abs: " << *new_abstraction << endl;
                res_abstractions.push_back(new_abstraction);
                finished = true;
            }
            if (finished || build_abstraction_dfs(new_abstraction, order,
                                                  explored_varsets, res_abstractions)) {
                if (c_release_memory) {
                    new_abstraction->release_memory();
                }
                return true; //Proved unsolvability
...
{
    Abstraction::build_atomic_abstractions(is_unit_cost(),
    ...
    for (size_t i = 0; i < atomic_abstractions.size(); ++i) {
        ...
        if (empty_label_shrink_strategy) {
            // Note: this works only for linear merging strategies, as the labels of an atomic abstraction' transitions
            // cannot be empty for the variables in the MAIN abstraction
            cout << "applying empty label shrinking" << endl;
            ...
            if (check_unsolvability) {
                ...
                const vector<int> &new_vars = g_causal_graph->get_predecessors(i);
                if (new_vars.empty()) {
        ...
        if (!atomic_abstractions[i]->is_irrelevant()) {
            atomic_abstractions[i]->compute_distances(prune_unreachable, prune_irrelevant,
                    continue_if_unsolvable, use_uniform_distances);
            if (!atomic_abstractions[i]->is_solvable() && !continue_if_unsolvable) {
                cout << "Number of merging steps: 0" << endl;
                cout << "Last abstraction step: -1/" << g_variable_domain.size() << " vars" <<
                     endl;
                return;
            }
    ...
    /*int num_irrelevant_variables = 0;
      ...
      cout << "Number of irrelevant variables: " << num_irrelevant_variables << endl;
      exit(1);*/
}
...
Abstraction *MergeAndShrinkHeuristic::build_abstraction(bool is_first)
{
    ...
    Abstraction::build_atomic_abstractions(
        is_unit_cost(), get_cost_type(), atom_abstractions);
    ...
    for (size_t i = 0; i < atom_abstractions.size(); ++i) {
        ...
        if (empty_label_shrink_strategy) {
            ...
            if (check_unsolvability) {
                ...
                if (new_vars.empty()) {
                    ...
                    // if its DTG is additionally a single SCC, we can ignore this variable as it will not change anything
                    // in terms of solvability
                    irrelevant_abstractions[i] = atom_abstractions[i]->is_scc();
                }
            }
        }
        atom_abstractions[i]->compute_distances(prune_unreachable, prune_irrelevant,
                                                continue_if_unsolvable, use_uniform_distances);
        ...
        if (!irrelevant_abstractions[i]) { // no need to shrink if abstraction will be ignored anyway
    ...
    while (!order->done()) {
        ...
        */
        ...
        int peak_memory = max(abstraction->get_peak_memory_estimate(),
                              other_abstraction->get_peak_memory_estimate());
        int max_abstract_states = max(abstraction->get_max_num_states(),
                                      other_abstraction->get_max_num_states());
        int max_abstract_transitions = max(abstraction->get_max_num_transitions(),
                                           other_abstraction->get_max_num_transitions());
        int overall_peak_memory = max(abstraction->get_overall_peak_memory_estimate(),
                                      other_abstraction->get_overall_peak_memory_estimate());
        int overall_max_abstract_states = max(abstraction->get_overall_max_num_states(),
                                              other_abstraction->get_overall_max_num_states());
        int overall_max_abstract_transitions = max(
        ...
        cout << "Estimated peak memory for this abstraction: " << peak_memory <<
             " bytes" << endl;
        cout << "Maximal number of states in this abstraction: " << max_abstract_states
             << endl;
        ...
        abstraction->set_overall_max_num_transitions(overall_max_abstract_transitions);
    ...
    if (use_uniform_distances && !is_unit_cost_problem()) {
        if (shrink_strategy->name() != "no_shrink" ||
            ...
            // force recalculation of distances and use normal action costs if so far BFS was forced and original actions are not unit-cost
        	force_recalculation = true;
        }
    }
    if (force_recalculation)
    	abstraction->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, false, true);
    else*/
    abstraction->compute_distances(prune_unreachable, prune_irrelevant,
                                   continue_if_unsolvable, use_uniform_distances);
    if (!abstraction->is_solvable() && !continue_if_unsolvable) {
    ...
    std::shared_ptr<ShrinkStrategy> def_shrink = ShrinkFH::create_default(abstraction->size());
    def_shrink->shrink(*abstraction, abstraction->size(), true);
    /*if (force_recalculation)
    	abstraction->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable);
    else*/
    abstraction->compute_distances(prune_unreachable, prune_irrelevant,
                                   continue_if_unsolvable, use_uniform_distances);
...
void MergeAndShrinkHeuristic::initialize()
{
    ...
    if (g_variable_domain.empty() || g_operators.empty()) {
        cout << "Problem detected as unsolvable by preproessor" << endl;
        return;
    }
    int peak_memory = 0;
    ...
    if (merge_strategy == MERGE_LINEAR_MULTIPLE_DFS) {
        ...
        for (int i = 0; i < abstraction_count; i++) {
            ...
            peak_memory = max(peak_memory, abstraction->get_overall_peak_memory_estimate());
            max_abstract_states = max(max_abstract_states,
                                      abstraction->get_overall_max_num_states());
            max_abstract_transitions = max(max_abstract_transitions,
                                           abstraction->get_overall_max_num_transitions());
    ...
    if (shrink_strategy->name() == "no_shrink" &&
        ...
        for (size_t i = 0; i < abstractions.size(); i++) {
            ...
            abstractions[i]->write_action_files(shrink_strategy->is_backward_pruned());
#ifdef DEBUG_ABSTRACTIONS
...
{
    if (check_unsolvability) {
        // in case we are only interested in detecting dead-ends, we can ignore the actual costs unless they are infinity
        for (size_t i = 0; i < abstractions.size(); i++) {
...
static std::shared_ptr<Heuristic> _parse(options::OptionParser &parser)
{
    ...
    parser.add_option<std::shared_ptr<ShrinkStrategy> >("shrink_strategy", "shrink strategy",
                                        "default");

    parser.add_list_option<std::shared_ptr<MergeCriterion>> ("merge_criteria",
    ...
    parser.add_option<bool>("prune_unreachable",
                            "prune unreachable states (WARNING: setting this to false it not fully supported)",
                            "true");
    parser.add_option<bool>("prune_irrelevant",
                            "prune irrelevant states (WARNING: setting this to false it not fully supported)",
                            "true");
    ...
    parser.add_option<int>("merge_size_limit",
                           "Maximum size of abstractions after a merge in MERGE_LINEAR_MULTIPLE_DFS",
                           "10000");
...
static std::shared_ptr<Heuristic> _parse_ownk(options::OptionParser &parser)
{
    ...
    if (!parser.dry_run()) {
        ...
        merge_criteria.push_back(std::make_shared<MergeCriterionMinSCC>(false, false));
        merge_criteria.push_back(std::make_shared<MergeCriterionGoal>());
        opts.set<std::vector<std::shared_ptr<MergeCriterion> > >("merge_criteria", merge_criteria);
        ...
        opts.set<std::shared_ptr<ShrinkStrategy>>("shrink_strategy",
...
static std::shared_ptr<Heuristic> _parse_owna(options::OptionParser &parser)
{
    ...
    if (!parser.dry_run()) {
        ...
        tmp1.set<int>("cost_type", 1);
        tmp1.set<int>("max_states", std::numeric_limits<int>::max());
        tmp1.set<int>("max_states_before_merge", std::numeric_limits<int>::max());
        tmp1.set<int>("greedy", 0);
        tmp1.set<int>("threshold", 1);
        tmp1.set<bool>("initialize_by_h", true);
        tmp1.set<bool>("group_by_h", false);
