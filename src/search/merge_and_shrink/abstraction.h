...
private:
    ...
    //count_transitions_var and count_transitions_var_empty. For each operator it has a set of
    //variables involved in its label.
    ...
    void apply_abstraction(vector<std::list<AbstractStateRef> > &collapsed_groups);
    typedef std::list<AbstractStateRef> EquivalenceClass;
    typedef std::vector<EquivalenceClass> EquivalenceRelation;
    bool apply_label_inheritance(const LabelReducer *reducer,
                                 bool new_approach = false);
    void comp_tc(AbstractStateRef v, int *current_index, vector<int> &index,
                 vector<AbstractStateRef> &nstack, vector<int> &cstack, vector<int> &C,
                 vector<int> &root, vector<vector<AbstractStateRef> > &successors,
                 int *component_count, vector<set<int> > &csucc, EquivalenceRelation &sccs);
...
public:
    ...
    void compute_distances(bool prune_unreachable, bool prune_irrelevant,
                           bool continue_if_unreachable = false, bool use_uniform_costs = false,
                           bool force_recomputations = false);
    void normalize(bool reduce_labels, const std::vector<bool> &caught_labels,
                   bool use_label_inheritance = false, bool new_approach = false);
    void release_memory();
    ...
    int set_goal_leading_actions(vector<bool> &labels,
                                 bool backward_pruning = false) const;
    int set_shortest_path_relevant_actions(vector<bool> &labels,
                                           bool backward_pruning = false);
    int set_path_relevant_actions(vector<bool> &labels,
                                  bool backward_pruning = false);
    ...
    void write_mip_file(const bool backward_pruning, const bool use_uniform_costs);
...
protected:
    ...
    virtual void apply_abstraction_to_lookup_table(const vector<AbstractStateRef>
            &abstraction_mapping);
    virtual int memory_estimate() const;
public:
