...
ShrinkEmptyLabels::ShrinkEmptyLabels(const options::Options &opts) : ShrinkStrategy(
        opts)
{
}
...
void ShrinkEmptyLabels::shrink(Abstraction &abs, int target, bool /*force*/)
{
     ...
     *     (a) all goal variables are in abstraction and
     *     (b) there is an empty-label path from s to g
     *     idea: set goal-status for all states already during SCC detection
     *     PETER: why only aggregate s with g and not all states that are marked as goal states?
     *     PETER: when we are at it, why not remove all outgoing transitions of goal states (we cannot
     *            leave them anyway any more)?
     *     PETER: actually, if we use the first optimization, we must use the second one as well, as otherwise
     *            unreachable states might suddenly become reachable resulting in an immense overhead
     *            in abstract states
     *     PETER: we might actually do this as a general optimization in Abstraction::normalize: whenever
     *            all goal variables are merged in, we can safely remove any transitions starting at goal states
     */
    ...
    /* this is a rather memory-inefficient way of implementing Tarjan's algorithm, but it's the best I got for now */
    vector<vector<int> > adjacency_matrix(num_states);
    const vector<const GlobalOperator *> &relevant_ops = abs.get_relevant_operators();
    for (size_t op_no = 0; op_no < relevant_ops.size(); op_no++) {
        if (!empty_label_operators[op_no]) {
            continue;
        }
        const vector<AbstractTransition> &transitions = abs.get_transitions_for_op(
                    get_op_index(relevant_ops[op_no]));
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        vector<int>::iterator it = unique(adjacency_matrix[i].begin(),
                                          adjacency_matrix[i].end());
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        {
            merge_and_shrink::find_sccs(stack, in_stack, final_sccs, adjacency_matrix, indices, lowlinks,
                        index, i, abs.get_all_goal_vars_in(), is_goal, state_to_scc);
        }
    }
        ...
        for (int i = 0; i < final_sccs.size(); i++) {
            ...
            for (; scc_it != final_scc.end(); scc_it++) {
                ...
                for (int j = 0; j < successors.size(); j++) {
                    ...
                    if (succ_scc < i)
                        /* add successors before current state: they were already handled */
                    {
                        final_scc.splice(scc_it, final_sccs[succ_scc]);
                    } else
                        /* add successors at end of current list: they still need to be handled */
                    {
...
void ShrinkEmptyLabels::determine_empty_label_operators(Abstraction &abs,
        vector<bool> &empty_label_operators)
{
...
static Plugin<ShrinkStrategy> _plugin("shrink_empty_labels", _parse);
