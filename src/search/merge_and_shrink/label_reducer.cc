...
namespace std {
template <>
struct hash<merge_and_shrink::OperatorSignature> {
    size_t operator()(const merge_and_shrink::OperatorSignature& sig) const
    {
        return sig.hash();
    }
};
...
OperatorSignature
LabelReducer::build_operator_signature(
    ...
    set<int>& vars_label_operator) const
{
    ...
    return OperatorSignature(
        preconditions, effects, op_cost, label_caught);
}

void
LabelReducer::statistics() const
{
