...
void RelaxationHeuristic::build_unary_operators(const GlobalOperator &op, int op_no) {
    ...
    for (unsigned i = 0; i < prevail.size(); i++) {
        assert(prevail[i].var >= 0 && prevail[i].var < static_cast<int>(g_variable_domain.size()));
        assert(prevail[i].val >= 0 && prevail[i].val < g_variable_domain[prevail[i].var]);
        precondition.push_back(&propositions[prevail[i].var][prevail[i].val]);
    }
    for (unsigned i = 0; i < pre_post.size(); i++) {
        assert(pre_post[i].var >= 0 && pre_post[i].var < static_cast<int>(g_variable_domain.size()));
        assert(pre_post[i].val >= 0 && pre_post[i].val < g_variable_domain[pre_post[i].var]);
        ...
        for (unsigned j = 0; j < eff_cond.size(); j++) {
            assert(eff_cond[j].var >= 0 && eff_cond[j].var < static_cast<int>(g_variable_domain.size()));
