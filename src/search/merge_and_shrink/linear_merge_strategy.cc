...
LinearMergeStrategy::LinearMergeStrategy(const std::vector <std::shared_ptr<MergeCriterion> >
        &merge_criteria,
        const MergeOrder &merge_order, bool is_first) :
    criteria(merge_criteria), order(merge_order)
{
...
{
    vector<int>::iterator position = find(remaining_vars.begin(),
                                          remaining_vars.end(), var_no);
                              ...
                              Abstraction *abstraction)
{
    ...
    if (!allow_incremental) {
        ...
        for (size_t v = 0; v < varset.size(); v++) {
            const vector<int> &new_vars = g_causal_graph->get_predecessors(varset[v]);
            for (size_t i = 0; i < new_vars.size(); ++i) {
                is_causal_predecessor[new_vars[i]] = true;
            }
        }
...
static PluginTypePlugin<MergeCriterion> _plugin_type("LinearMergeCriterion", "");
static Plugin<MergeCriterion> _plugin_cg("cg",   _parse<MergeCriterionCG>);
static Plugin<MergeCriterion> _plugin_goal("goal",
        _parse<MergeCriterionGoal>);
static Plugin<MergeCriterion> _plugin_scc("scc",   _parse_scc);
static Plugin<MergeCriterion> _plugin_scc_no_level("scc_no_level",
        _parse_scc_no_level);
static Plugin<MergeCriterion> _plugin_empty("empty",
        _parse<MergeCriterionEmpty>);
static Plugin<MergeCriterion> _plugin_empty_goal("empty_goal",
        _parse<MergeCriterionEmptyGoal>);
static Plugin<MergeCriterion> _plugin_num("num",   _parse<MergeCriterionNum>);
static Plugin<MergeCriterion> _plugin_rel("relevant",
