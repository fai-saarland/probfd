...
protected:
    ...
    Label(int id, int cost, const std::vector<GlobalCondition> &prevail,
          const std::vector<GlobalEffect> &pre_post);
    virtual ~Label() {}
    virtual void update_root(CompositeLabel *new_root) = 0;
public:
    const std::vector<GlobalCondition> &get_prevail() const
    {
        return prevail;
    }
    const std::vector<GlobalEffect> &get_pre_post() const
    {
        return pre_post;
    }
    int get_id() const
...
public:
    OperatorLabel(int id, int cost, const std::vector<GlobalCondition> &precond,
                  const std::vector<GlobalEffect> &pre_post);
