#include "search_engines/iterated_search.h"
...
IteratedSearch::IteratedSearch(
    ...
    : SearchEngine(opts)
    , engine_configs(opts.get_list<options::ParseTree>("engine_configs"))
    , registry(registry)
...
IteratedSearch::get_search_engine(int engine_configs_index)
{
    options::OptionParser parser(
        engine_configs[engine_configs_index],
        registry,
        predefinitions,
        false);
    std::shared_ptr<SearchEngine> engine =
        parser.start_parsing<std::shared_ptr<SearchEngine>>();
...
std::shared_ptr<SearchEngine> IteratedSearch::create_current_phase(int p)
{
...
{
    auto current_search = create_current_phase(phase);
    ...
    if (last_phase_found_solution) {
        ...
        if (plan_cost < best_bound) {
            plan_manager.save_plan(found_plan, true);
...
static std::shared_ptr<SearchEngine> _parse(options::OptionParser& parser)
{
    ...
    parser.document_note(
        ...
        "Adding heuristic caching is [issue108 "
        "http://issues.fast-downward.org/issue108].");
    parser.document_note(
        "Note 2",
        "The configuration\n```\n"
        "--search \"iterated([lazy_wastar(merge_and_shrink(),w=10), "
        "lazy_wastar(merge_and_shrink(),w=5), "
        "lazy_wastar(merge_and_shrink(),w=3), "
        "lazy_wastar(merge_and_shrink(),w=2), "
        "lazy_wastar(merge_and_shrink(),w=1)])\"\n"
        "```\nwould perform the preprocessing phase of the merge and shrink "
        "heuristic "
        ...
        "\"iterated([lazy_wastar(h,w=10), lazy_wastar(h,w=5), "
        "lazy_wastar(h,w=3), "
    ...
    parser.add_list_option<options::ParseTree>(
        "engine_configs",
    ...
    opts.verify_list_non_empty<options::ParseTree>("engine_configs");

    if (parser.help_mode()) {
        return 0;
    } else if (parser.dry_run()) {
        // check if the supplied search engines can be parsed
        vector<options::ParseTree> configs =
            opts.get_list<options::ParseTree>("engine_configs");
        for (size_t i = 0; i < configs.size(); ++i) {
            options::OptionParser test_parser(
                configs[i],
                parser.get_registry(),
                parser.get_predefinitions(),
                true);
            test_parser.start_parsing<std::shared_ptr<SearchEngine>>();
        }
        return 0;
    } else {
        std::shared_ptr<IteratedSearch> engine =
            std::make_shared<IteratedSearch>(
                opts,
                parser.get_registry(),
                parser.get_predefinitions());
