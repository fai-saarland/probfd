...
void LazySearch::initialize()
{
    ...
    cout << "Conducting lazy best first search, (real) bound = " << bound
         << endl;
...
static std::shared_ptr<SearchEngine>
_parse_weighted_astar(options::OptionParser& parser)
{
    ...
    if (!parser.dry_run()) {
        ...
        for (size_t i = 0; i < evals.size(); ++i) {
            ...
            if (opts.get<int>("w") == 1) {
                ...
                std::shared_ptr<WeightedEvaluator> w =
                    std::make_shared<WeightedEvaluator>(
                        evals[i],
                        opts.get<int>("w"));
                sum_evals.push_back(w);
            }
            std::shared_ptr<SumEvaluator> f_eval =
