...
TieBreakingOpenList<Entry>::TieBreakingOpenList(const options::Options &opts)
...
INSTANTIATE_OPEN_LIST(TieBreakingOpenList)

TieBreakingOpenListFactory::TieBreakingOpenListFactory(
        const std::vector<std::shared_ptr<Evaluator> > &evals,
                bool preferred_only,
                bool unsafe_pruning)
    : FallthroughOpenListFactory<TieBreakingOpenList>(options::Options())
{
    opts.set<std::vector<std::shared_ptr<Evaluator> > >("evals", evals);
    opts.set<bool>("pref_only", preferred_only);
    opts.set<bool>("unsafe_pruning", unsafe_pruning);
}

std::shared_ptr<OpenListFactory> _parse(options::OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return std::make_shared<TieBreakingOpenListFactory>(opts);
}
