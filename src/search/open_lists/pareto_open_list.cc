...
ParetoOpenListFactory::ParetoOpenListFactory(const plugins::Options& options)
    : options(options)
{
}
...
ParetoOpenListFactory::create_state_open_list() {
    return std::make_unique<ParetoOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
ParetoOpenListFactory::create_edge_open_list() {
    return std::make_unique<ParetoOpenList<EdgeOpenListEntry>>(options);
}
