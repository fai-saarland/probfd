...
template<class Entry>
ParetoOpenList<Entry>::ParetoOpenList(const std::vector<std::shared_ptr<Evaluator>> &evals,
...
template<class Entry>
ParetoOpenList<Entry>::ParetoOpenList(const options::Options &opts)
    : OpenList<Entry>(opts.get<bool>("pref_only")),
      state_uniform_selection(opts.get<bool>("state_uniform_selection")),
      evaluators(opts.get_list<std::shared_ptr<Evaluator>>("evals")) {
    last_evaluated_value.resize(evaluators.size());
}
...
INSTANTIATE_OPEN_LIST(ParetoOpenList)

ParetoOpenListFactory::ParetoOpenListFactory(
        const std::vector<std::shared_ptr<Evaluator> > &evals,
                   bool preferred_only,
                   bool state_uniform_selection_)
    : FallthroughOpenListFactory(options::Options())
{
    opts.set<std::vector<std::shared_ptr<Evaluator> >>("evals", evals);
    opts.set<bool>("pref_only", preferred_only);
    opts.set<bool>("state_uniform_selection", state_uniform_selection_);
}
...
static std::shared_ptr<OpenListFactory> _parse(options::OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return std::make_shared<ParetoOpenListFactory>(opts);
}
