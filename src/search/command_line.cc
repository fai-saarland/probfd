...
std::shared_ptr<SolverInterface> parse_cmd_line(
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--if-unit-cost") {
            ...
            active = true;
        } else if (arg == "--horizon" || arg == "--budget" || arg == "--step-bound") {
            active = true;
            if (i + 1 == argc) {
                throw ArgError("missing argument after " + arg);
            }
    ...
    if (!dry_run) {
        ...
        if (build_successor_generator) {
            ...
            successor_generator::g_successor_generator = std::make_shared<successor_generator::SuccessorGenerator<const GlobalOperator *>>();
            cout << "done! [t=" << utils::g_timer << "]" << endl;
        }
        // if (expected_cost) {
