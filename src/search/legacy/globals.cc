...
void dump_initial_state()
{
    ...
    for (size_t i = 0; i < g_initial_state_data.size(); ++i)
        cout << "  " << g_variable_name[i] << ": " << g_initial_state_data[i]
             << endl;
}
...
void read_axioms(istream& in)
{
    ...
    g_axiom_evaluator = new legacy::AxiomEvaluator;
}
...
GlobalState g_initial_state()
{
    if (g_cached_initial_state == nullptr) {
        g_cached_initial_state =
            new PackedStateBin[g_state_packer->get_num_bins()];
        fill_n(g_cached_initial_state, g_state_packer->get_num_bins(), 0);
        for (size_t i = 0; i < g_initial_state_data.size(); ++i) {
            g_state_packer->set(
                g_cached_initial_state,
                i,
                g_initial_state_data[i]);
        }
    }
...
legacy::AxiomEvaluator* g_axiom_evaluator;
