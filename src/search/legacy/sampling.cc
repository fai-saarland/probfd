...
static GlobalState sample_state_with_random_walk(
    ...
    const successor_generator::SuccessorGenerator<const GlobalOperator*>&
        successor_generator,
    int init_h,
    ...
    function<bool(GlobalState)> is_dead_end)
{
    ...
    for (int j = 0; j < length; ++j) {
        ...
        if (applicable_operators.empty()) {
            ...
            const GlobalOperator* random_op_id =
                *rng.choose(applicable_operators);
...
GlobalState
RandomWalkSampler::sample_state(int init_h, const DeadEndDetector& is_dead_end)
    const
{
