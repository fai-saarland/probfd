...
// TODO if the input file format has been changed, we would need something like
// this Effect::Effect(istream &in) {
//    int cond_count;
//    in >> cond_count;
...
void GlobalOperator::read_pre_post(istream& in)
{
    ...
    in >> var >> pre >> post;
    if (pre != -1) preconditions.push_back(GlobalCondition(var, pre));
