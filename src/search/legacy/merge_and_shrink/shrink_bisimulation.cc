...
struct Signature {
    ...
    void dump() const
    {
        cout << "Signature(h_and_goal = " << h_and_goal << ", group = " << group
             << ", state = " << state << ", succ_sig = [";
...
void ShrinkBisimulation::shrink(Abstraction& abs, int target, bool force)
{
    ...
    if (abs.size() == 1 && greediness != NOT_GREEDY) {
        cout
            << "Special case: do not greedily bisimulate an atomic abstraction."
            << endl;
        return;
    }
...
void ShrinkBisimulation::shrink_atomic(Abstraction& abs)
{
    ...
    if (abs.size() != old_size) {
        cout << "Atomic abstraction simplified "
             << "from " << old_size << " to " << abs.size() << " states."
             << endl;
    }
}
    ...
    vector<int>& state_to_group)
{
    ...
    signatures.push_back(
        Signature(infinity, false, -1, SuccessorSignature(), -1));
    ...
    for (int op_no = 0; op_no < num_ops; ++op_no) {
        ...
        for (size_t i = 0; i < transitions.size(); ++i) {
            ...
            if (abs.get_all_goal_vars_in() && abs.is_goal_state(trans.src)) {
                // is goal state and will always remain one -> we can ignore all
                // outgoing transitions, as we can never leave this state;
                // should help aggregating more states, as all goal states
    ...
    for (size_t i = 0; i < signatures.size(); ++i) {
        ...
        succ_sig.erase(
            ::unique(succ_sig.begin(), succ_sig.end()),
            succ_sig.end());
    }
    ...
    EquivalenceRelation& equivalence_relation)
{
    ...
    // assert(num_groups <= target_size); // TODO: We currently violate this;
    // see issue250
...
void ShrinkBisimulation::set_labels_from_abstraction(
    Abstraction& abs,
    ...
    int type)
{
    ...
    if (type == 0) {
        ...
        cerr << "unknown type " << type
             << " for setting labels from abstraction" << endl;
        exit(1);
    }
}
