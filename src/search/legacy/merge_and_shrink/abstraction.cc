...
#include "legacy/merge_and_shrink/utils.h"
...
Abstraction::Abstraction(bool is_unit_cost_, OperatorCost cost_type_)
    : is_unit_cost(is_unit_cost_)
    , cost_type(cost_type_)
    , irrelevant(false)
    , are_labels_reduced(false)
    , peak_memory(0)
    , max_num_states(0)
    , max_num_transitions(0)
    , overall_peak_memory(0)
    , overall_max_num_states(0)
    , overall_max_num_transitions(0)
...
void Abstraction::compute_distances(
    ...
    bool continue_if_unsolvable,
    bool use_uniform_costs,
    bool force_recomputation)
{
    ...
    init_distances.resize(
        num_states,
        prune_unreachable ? infinity : infinity - 1);
    goal_distances.resize(
        num_states,
    ...
    if (prune_unreachable || prune_irrelevant) {
        ...
        if (unreachable_count || irrelevant_count) {
            ...
            std::shared_ptr<ShrinkStrategy> shrink_temp =
                ShrinkFH::create_default(num_states);
            shrink_temp->shrink(*this, num_states, true);
#ifdef DEBUG_ABSTRACTIONS
            /* We don't need to normalize here semantically, but if we don't, we
               won't get transitions_by_op[i] in sorted order which will cause
               warnings in the dump() method. To silence those warning,
               uncomment the following line. Since it changes behaviour, which
               is never a good thing for debug settings to do, it's commented
               out by default. */
            // normalize(false, vector<bool>());
#endif
        }
    } else {
        // skip unreachable/irrelevant pruning, but at least check for initial
        ...
        if (goal_distances[init_state] >= infinity - 1) {
            ...
            cout << tag() << "initial state irrelevant; task unsolvable"
                 << endl;
    ...
    vector<int>& other_distances)
{
    while (!queue.empty()) {
        ...
        for (size_t i = 0; i < graph[state].size(); i++) {
            ...
            if (!other_distances.empty() &&
                other_distances[successor] == infinity) {
...
{
    assert(!init_distances
                .empty()); // make sure that forward search is performed first
    ...
    const bool find_only_one)
{
    ...
    for (size_t i = 0; i < successors.size(); i++) {
        ...
        /*cout << "lowlink of state " << state << ": " << lowlinks[state] <<
         * endl;*/
        /* update goal status */
...
bool Abstraction::is_scc() const
{
    ...
    find_sccs(
        ...
        state_to_scc,
        true);
    if (final_sccs[0].size() ==
        (size_t)num_states) { // all states in a single SCC
        return true;
    }
    return false;
}
    ...
    EquivalenceRelation& sccs)
{
    ...
    for (size_t i = 0; i < v_succs.size(); i++) {
        ...
        } else if (w_unvisited || index[w] < index[v]) { // if (v,w) not a
                                                         // forward edge
...
bool Abstraction::apply_label_inheritance(
    ...
    bool new_approach)
{
    ...
    if (new_approach) {
        ...
        if (reducer) {
            ...
            while (non_empty_transition) {
                ...
                // for (size_t op_no = 0; op_no < transitions_by_op.size();
                // ++op_no) {
                empty_label_transition[op_no] =
                    reducer->get_empty_label_operators(op_no);
                ...
                for (size_t j = 0; j < transitions.size(); j++) {
                    ...
                    all_successors[trans.src].push_back(
                        make_pair(trans.target, op_no));
#endif
                }
                non_empty_transition = non_empty_transition->succ;
            }
             ...
             * if some variable is in pre or eff that is not in abstraction it
             * is not an empty label operator store all successors of a state in
             * additional array (for faster access later on)
             */
            ArrayList* non_empty_transition = first_non_empty_transition;
            while (non_empty_transition) {
                int op_no = non_empty_transition->op_no;
        ...
        // cout << "step 2: calculate transitive closure over empty-label
        // transitions" << endl;
        vector<AbstractStateRef> nstack;
        vector<int> cstack;
        vector<int> C;
        ...
        for (size_t op_no = 0; op_no < transitions_by_op.size(); ++op_no) {
            ...
            for (size_t j = 0; j < transitions.size(); j++) {
                const AbstractTransition& trans = transitions[j];
                all_successors2[trans.src].insert(
                    make_pair(trans.target, op_no));
            }
        }
        for (int s = 0; s < num_states; s++) {
            ...
            while (csucc_it != csucc_set.end()) {
                ...
                const set<pair<AbstractStateRef, int>>& all_succs =
                    all_successors2[succ];
                set<pair<AbstractStateRef, int>>::const_iterator succ_it =
                    all_succs.begin();
                while (succ_it != all_succs.end()) {
                    ...
                    vector<AbstractTransition>& transitions =
                        transitions_by_op[trans_pair.second];
        ...
        for (AbstractStateRef s = 0; s < num_states; s++) {
            /* copy all outgoing transitions of nodes in successor SCCs (if SCC
             * has more than 1 node, this will include itself) */
            int scc_id = C[s];
        ...
        /*cout << "step 3: calculate set of predecessors based on empty-label
        transitions" << endl; vector<set<AbstractStateRef> >
        predecessors(num_states); for (int i = 0; i < num_states; i++) { int
        scc_index = C[i]; const vector<AbstractStateRef>& scc = sccs[scc_index];
                ...
                set<int>::const_iterator csucc_it = csucc_set.begin();
                while (csucc_it != csucc_set.end()) {
                        const vector<AbstractStateRef>& succ_scc =
        sccs[*csucc_it]; for (int j = 0; j < succ_scc.size(); j++) {
        ...
        vector<vector<AbstractTransition> >
        new_transitions_by_op(transitions_by_op.size()); for (int op_no = 0;
        op_no < transitions_by_op.size(); op_no++) { vector<AbstractTransition>
        &transitions = transitions_by_op[op_no]; vector<AbstractTransition>
        &new_transitions = new_transitions_by_op[op_no]; for (int i = 0; i <
        transitions.size(); i++) { const AbstractTransition &trans =
        transitions[i]; const set<AbstractStateRef>& pred_set =
        predecessors[trans.src]; set<AbstractStateRef>::const_iterator pred_it =
        pred_set.begin(); while (pred_it != pred_set.end()) { AbstractTransition
        new_trans(*pred_it, trans.target); new_transitions.push_back(new_trans);
                                pred_it++;
                        }
                }
        ...
        if (reducer) {
            ...
            while (non_empty_transition) {
                ...
                for (size_t j = 0; j < transitions.size(); j++) {
                    const AbstractTransition& trans = transitions[j];
                    successors[trans.src].push_back(
                        make_pair(trans.target, op_no));
                }
                non_empty_transition = non_empty_transition->succ;
            }
        ...
        // cout << "step 2: find all reachable states and (inherited)
        // transitions for all states" << endl;
        ...
        for (AbstractStateRef s = 0; s < num_states; s++) {
            ...
            while (!states.empty()) {
                ...
                states.pop_front();
                const vector<pair<AbstractStateRef, int>>& succs =
                    successors[state];
                int size = succs.size();
                for (int i = 0; i < size; i++) {
                    ...
                    /*cout << "    successor " << i << ": " << statep << " (by
                     * operator " << op_no << (empty_label_transition[op_no] ?
                     * "*" : "") << ")" << endl;*/
                    if (s != state) {
                        AbstractTransition trans(s, statep);
                        transitions_by_op[op_no].push_back(trans);
                        /*cout << "      added new transition " << s << " -> "
                         * << statep << endl;*/
                    }
                    if (!reachable_states[statep]) {
                        if (empty_label_transition[op_no]) {
                            ...
                            if (goal_states[statep]) {
                                goal_states[s] = true;
                                // break; /* no need to continue here; we do not
                                // care about outgoing transitions in goal
                                // states */
                            }
    ...
    // label inheritance is not f-preserving; must recalculate distances later
    // on
    clear_distances();
...
//             	if (goal_states[t.src]) // we want to remove the outgoing
//             actions of all goal states 		continue;
    ...
    bool new_approach)
{
    ...
    */
    ...
    while (non_empty_transition) {
        ...
        if (reduced_op_no != op_no) {
            ...
            if (!non_empty_transitions[reduced_op_no]) {
                ...
                if (prev_pos == -1) {
                    ...
                    new_non_empty_transition->prev =
                        non_empty_transitions[prev_pos];
                    new_non_empty_transition->succ =
                        non_empty_transitions[prev_pos]->succ;
                    non_empty_transitions[prev_pos]->succ =
                        new_non_empty_transition;
                }
                if (new_non_empty_transition->succ) {
                    new_non_empty_transition->succ->prev =
                        new_non_empty_transition;
                }
        ...
        for (size_t i = 0; i < transitions.size(); i++) {
            ...
            /*if (all_goal_vars_in && goal_states[t.src]) {
                // goal state will always remain goal state, which we cannot
            leave continue;
            }*/
            target_buckets[t.target].push_back(make_pair(t.src, reduced_op_no));
        }
        vector<AbstractTransition>().swap(transitions);
        //}
        // non_empty_transition = non_empty_transition->succ;
    ...
    if (use_label_inheritance) {
        // Peter: statistics has as side-effect that they calculate the maximum
        // number of states and transitions. For that need to call statistics
        // only whenever these sizes can increase. That can only happen in case
        // of label inheritance, giving us new transitions; in all other cases
        // the sizes can only decrease, and we do not care about this.
        // statistics(false);
        /*bool did_aggregate_states =*/apply_label_inheritance(
            reducer,
            new_approach);
        /*if (reducer && did_aggregate_states) { // shrinking cannot change the
        output of the label reducer! delete reducer; reducer = new
        LabelReducer(relevant_operators, varset, cost_type, caught_labels);
        }*/
    ...
    vector<Abstraction*>& result)
{
    ...
    for (int var_no = 0; var_no < var_count; var_no++) {
        result.push_back(
            new AtomicAbstraction(is_unit_cost, cost_type, var_no));
    }
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); op_no++) {
        ...
        for (int var : relevant_variables) {
            if (precondition[var] == effect[var]) {
                ...
                for (int value = pre_value_min; value < pre_value_max;
                     value++) {
                    /*if (abs->get_all_goal_vars_in() &&
                    abs->is_goal_state(value)) {
                        // goal state will always remain goal state, which we
                    cannot leave continue;
                    }*/
    ...
    for (size_t i = 0; i < result.size(); i++) {
        ...
        // Peter: store which transitions are not empty for faster access in
        ...
        for (size_t op_no = 0; op_no < g_operators.size(); op_no++) {
            if (abs->transitions_by_op[op_no].empty()) {
                ...
                // this element does exist; set its predecessor to the last
                // non-empty element, its successor to the last element, its
                // predecessor's successor to this, and the last element's
                // predecessor to this
                ...
                if (last_non_empty) {
                    ...
                    abs->first_non_empty_transition =
                        abs->non_empty_transitions[op_no];
                }
...
CompositeAbstraction::CompositeAbstraction(
    ...
    : Abstraction(is_unit_cost, cost_type)
{
    cout << "Merging " << abs1->description() << " and " << abs2->description()
         << endl;
    ...
    init_state = PRUNED_STATE; // default value -- in case it is pruned in one
                               // of the abstractions
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); op_no++) {
        ...
        if (relevant1 || relevant2) {
            // label reduction of abs2 not necessary, as all actions being
            // reduced to the same action in abs1 must be equal in abs2
            vector<AbstractTransition>& transitions = transitions_by_op[op_no];
                ...
                for (size_t i = 0; i < bucket1.size(); i++) {
                    ...
                    for (size_t j = 0; j < bucket2.size(); j++) {
                        ...
                        /*if (newly_all_goal_vars_in && goal_states[src]) {
                                // goal state will always remain goal state,
                        which we cannot leave continue;
                        }*/
...
{
    ostringstream s;
    s << "abstraction (" << varset.size() << "/" << g_variable_domain.size()
      << " vars)";
    return s.str();
}

AbstractStateRef
AtomicAbstraction::get_abstract_state(const GlobalState& state) const
{
...
AbstractStateRef
CompositeAbstraction::get_abstract_state(const GlobalState& state) const
{
    ...
    vector<std::list<AbstractStateRef>>& collapsed_groups)
{
    ...
    if (init_distances.empty()) { // distances already cleared; cannot copy any
                                  // distance infos
...
void Abstraction::statistics(bool include_expensive_statistics) const
{
    ...
    cout << "/" << num_transitions << " arcs, " << memory << " bytes" << endl;
    cout << tag();
    if (max_h == DISTANCE_UNKNOWN) {
...
/* Additional dump method added by Michael.
   TODO: If we're going to keep this, it must be consolidated with the above
   dump() method.
*/
...
void Abstraction::dump(ostream& ou) const
{
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        bool is_goal = (goal_states[i] == true);
        ou << "    node" << i
           << " [shape = " << (is_goal ? "doublecircle" : "circle")
           << ",label=\"" << i << "(" << get_goal_distance(i) << ")\"];"
           << endl;
        if (is_init) {
...
int Abstraction::set_goal_leading_actions(
    vector<bool>& labels,
    bool backward_pruning) const
{
    ...
    while (non_empty_transition) {
        ...
        if (last_computation_used_uniform_costs) {
            ...
            op_cost = get_adjusted_action_cost(
                g_operators[op_no],
        ...
        for (size_t i = 0; i < trans.size(); i++) {
            int src = trans[i].src;
            int target = trans[i].target;
            // Check if the value of the target is less or equal the value of
            // the source.
            ...
            //        	") to " << target << " (h*="<< target_g <<") with cost "
            //        << op_cost << endl;
...
int Abstraction::set_shortest_path_relevant_actions(
    vector<bool>& labels,
    bool backward_pruning)
{
    ...
    if (!last_computation_used_uniform_costs) {
        // we need uniform cost actions to properly operate here, so we have to
        // recalculate the distances accordingly
    ...
    // calculate outgoing transitions for each state - disregarding those states
    // with too high distance from goal
    for (size_t op_no = 0; op_no < g_operators.size(); op_no++) {
        const vector<AbstractTransition>& transitions =
            transitions_by_op[op_no];
        int op_cost = 1;
    ...
    /*
     * Original idea: For each state, check if any outgoing action has been
     * marked. If not, mark the first one
     */
    /*for (int i = 0; i < num_states; i++) {
        //::sort(outgoing_transitions[i].begin(),
    ...
    /*
     * New idea: Start with all operators that are strictly necessary, i.e., for
     * which there is some state that has only this as relevant outgoing
     * transition. Then continue with those operators that are relevant for the
     * highest number of states.
     */
    mark_relevant_operators(
        num_states,
    ...
    if (recalculated_distances) {
        // can be only because outside distances not computed by means of
...
int Abstraction::set_path_relevant_actions(
    vector<bool>& labels,
    bool /*backward_pruning*/)
{
    utils::Timer timer;
    ...
    while (num_states_done != num_states) {
        // cout << "next iteration; " << num_states_done << " of " << num_states
        // << " states already done." << endl; cout << "                " <<
        // num_actions << " of " << g_operators.size() << " actions caught." <<
        // endl;
        //  generate new states and update operators' marking counts
        list<int>::iterator queue_it = queue.begin();
        /*// go through queue once to keep only the undone states and the done
         * states with undone predecessors*/
        // go through queue only once and update counts of operators on the fly
        // (it is only an approximation anyway)
        while (queue_it != queue.end()) {
            int state = *queue_it;
            if (states_done[state]) {
                // some of state's outgoing operators are already caught; can
                // insert all predecessors into queue
                ...
                if (all_preds_done) {
                    // all predecessors are already done, so we can remove state
                    // from the queue
                    queue_it = queue.erase(queue_it);
                } else {
                    // not all predecessors done yet, so we have to keep state
                    // in the queue in order to properly update the counts of
                    // all operators
                    queue_it++;
                }
            } else {
                // no outgoing operators caught yet; must update operators'
                // marking counts for this state
                /*for (int i = 0; i < backward_graph[state].size(); i++) {
                        const pair<int, int> &pred_trans =
                backward_graph[state][i]; int pred = pred_trans.first; int op_no
                = pred_trans.second; if (!states_done[pred]) {
                                num_relevant_starting_states[op_no]++;
                        }
                }*/
                queue_it++;
        ...
        // go through queue a second time to get the correct counts for the
        operators while (queue_it != queue.end()) { int state = *queue_it; if
        (states_done[state]) {
                        // some of state's outgoing operators are already
        caught; can insert all predecessors into queue bool all_preds_done =
        true; for (int i = 0; i < backward_graph[state].size(); i++) { const
        pair<int, int> &pred_trans = backward_graph[state][i]; int pred =
        pred_trans.first; if (!states_in_open_closed[pred]) { cerr << "This
        cannot happen!" << endl; states_in_open_closed[pred] = true;
                                        queue.push_back(pred);
                                }
                                if (!states_done[pred]) {
                                        int op_no = pred_trans.second;
                                        if (labels[op_no]) {
                                                cerr << "This cannot happen!" <<
        endl; states_done[pred] = true; num_states_done++; } else {
                                                num_relevant_starting_states[op_no]++;
                                                all_preds_done = false;
                                        }
                                }
                        ...
                        if (all_preds_done) {
                                ...
                                // all predecessors are already done, so we can
        remove state from the queue queue_it = queue.erase(queue_it); } else {
                                // not all predecessors done yet, so we have to
        keep state in the queue in order to properly update
                                // the counts of all operators
                                queue_it++;
                        }
                } else {
                        // no outgoing operators caught yet; must update
        operators' marking counts for this state queue_it++;
                }
        }*/
        ...
        int max_index = -1;
        for (size_t op_no = 0; op_no < g_operators.size(); op_no++) {
            if (num_relevant_starting_states[op_no] > max_num &&
                !labels[op_no]) {
        ...
        /*// push newly marked states into queue and update operators' marking
        counts const vector<AbstractTransition> &transitions =
        transitions_by_op[max_index]; for (int i = 0; i < transitions.size();
        i++) { const AbstractTransition &trans = transitions[i];
    ...
    /*while (!queue.empty()) {
        ...
        for (int i = 0; i < graph[state].size(); i++) {
            int successor = graph[state][i];
            if (!other_distances.empty() && other_distances[successor] ==
    infinity) {
                // state already proven to be irrelevant
                continue;
            }
    ...
    vector<set<AbstractStateRef> > starting_states(g_operators.size());
    // calculate outgoing transitions for each state - disregarding those states
    with too high distance from goal for (int op_no = 0; op_no <
    g_operators.size(); op_no++) { const vector<AbstractTransition> &transitions
    = transitions_by_op[op_no]; int op_cost = 1; for (int i = 0; i <
    transitions.size(); i++) { const AbstractStateRef &src = transitions[i].src;
            int src_g = get_goal_distance(src);
            if ((src_g == infinity) *//*||
...
for (in_op_it = ingoing_transitions.begin(); in_op_it != ingoing_transitions.end(); in_op_it++) {
    ...
    if (num_states_done < num_states) {
        for (int op_no = 0; op_no < g_operators.size(); op_no++) {
                const vector<AbstractTransition> &transitions =
    transitions_by_op[op_no]; for (int i = 0; i < transitions.size(); i++) {
                        const int target = transitions[i].target;
                        if (states_done[target]) {
                                const int src = transitions[i].src;
                                pair<set<int>::iterator, bool> result =
    starting_states[op_no].insert(src); if (result.second) { // state was not in
    list before num_starting_states[op_no]++; if (num_starting_states[op_no] >
    max_num) { max_num = num_starting_states[op_no];
                                                //max_index = op_no;
                                        }
    ...
    transitions.size(); i++) { const AbstractStateRef &src = transitions[i].src;
            int src_g = get_goal_distance(src);
            if ((src_g == infinity) ||
                        (init_state != PRUNED_STATE && backward_pruning && src_g
    > get_goal_distance(init_state))) { continue;
            }
     ...
     * highest number of states.
     */
    // mark_relevant_operators(num_states, outgoing_transitions,
    // starting_states, labels, num_actions);
...
void Abstraction::get_goal_leading_actions(set<int>& ops) const
{
    ...
    while (non_empty_transition) {
        ...
        for (size_t i = 0; i < trans.size(); i++) {
            ...
            if (get_goal_distance(src) ==
                (get_goal_distance(target) + op_cost)) {
                ops.insert(op_no);
                break;
            }
        }
    ...
    const bool use_uniform_costs)
{
    ...
    ofstream out(
        use_uniform_costs ? "necessary_actions_uniform.cplex"
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        if (!outgoing_transitions[i].empty()) {
            ...
            out << "A"
                << outgoing_transitions[i][outgoing_transitions[i].size() - 1]
                << " >= 1" << endl;
        }
    }
    /*out << "Bounds" << endl;
...
{
    cout << "writing partial MaxSAT file for finding actions preserving "
            "optimal solutions"
         << endl;
    ...
    // if (backward_pruning) {
    //  prune all states with distance infinity and, in case of
    //  backward_pruning, all with too big distance
    ...
    // find maxDist by calculating abstraction based on uniform costs and
    // checking for maximal h-value
    ...
    out << "p wcnf " << num_variables << " " << num_clauses << " " << top
        << endl;
    ...
    // set all but the first iteration's states based on their outgoing
    // transitions
    offset = offset_r_variables;
    for (int s = 0; s < num_states; s++) {
        for (int k = 1; k <= maxDist; k++) {
            ...
            for (size_t t = 0; t < transitions.size(); t++) {
                out << " "
                    << (offset_t_variables + (k - 1) * num_transitions +
                        transitions[t]);
            }
            out << " 0" << endl;
        }
    ...
    // set transitions based on their target value from the previous transition
    // and the value of the corresponding operator
    ...
    for (size_t o = 0; o < g_operators.size(); o++) {
        ...
        for (size_t i = 0; i < transitions.size(); i++) {
            ...
            for (int k = 1; k <= maxDist; k++) {
                out << top << " -"
                    << (offset + (k - 1) * num_transitions + trans_index);
                out << " "
                    << offset_r_variables + (k - 1) * num_states + target;
                out << " 0" << endl;
                out << top << " -"
                    << (offset + (k - 1) * num_transitions + trans_index);
...
// void Abstraction::count_transitions_var_empty_goal(vector<double> & count)
// const {
//   for(int i = 0; i < vars_label_operators.size(); ++i){
