...
std::string MultipleShrinkingStrategy::name() const
{
...
bool MultipleShrinkingStrategy::reduce_labels_before_shrinking() const
{
    bool res = true;
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
void MultipleShrinkingStrategy::shrink(
    Abstraction& abs,
    int threshold,
    bool force)
{
...
void MultipleShrinkingStrategy::shrink_atomic(Abstraction& abs1)
{
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
void MultipleShrinkingStrategy::shrink_before_merge(
    Abstraction& abs1,
    Abstraction& abs2)
{
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
bool MultipleShrinkingStrategy::is_goal_leading() const
{
    bool res = true;
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
bool MultipleShrinkingStrategy::is_backward_pruned() const
{
    bool res = true;
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
bool MultipleShrinkingStrategy::require_distances() const
{
    bool res = true;
    for (unsigned i = 0; i < strategies_.size(); ++i) {
...
static Plugin<ShrinkStrategy> _plugin("legacy_shrink_multiple", _parse);
