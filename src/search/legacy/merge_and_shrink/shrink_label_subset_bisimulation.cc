    ...
    , max_states_before_catching(opts.get<int>("max_states_before_catching"))
{
    ...
    if (goal_leading == ALL_ACTIONS) {
        ...
        goal_leading == GOAL_LEADING_FROM_THRESHOLD ||
...
void ShrinkLabelSubsetBisimulation::dump_strategy_specific_options() const
{
    ...
    if (goal_leading == ALL_ACTIONS || goal_leading == ALL_GOAL_LEADING ||
        goal_leading == BACKWARD_PRUNED_GOAL_LEADING ||
        goal_leading == APPROXIMATED_BY_H_MAX ||
        goal_leading == APPROXIMATED_BY_H_MAX_BACKWARD_PRUNED ||
    ...
    //    cout << "Read goal leading actions "<< to_catch << " out of total
    //    number of actions " << g_operators.size() << endl;
    ...
    Abstraction& abs2)
{
    ...
    // If it is reached, the goal leading actions for the abstraction that
    // reached the limit are found and stored. From this point on every shrink
    // uses this set of actions as an approximation for the actual goal leading
    // actions
    // if (!goal_leading_threshold_reached && (new_size1 != abs1.size() ||
    ...
    if (!goal_leading_threshold_reached &&
        ...
        //    	cout <<
        //    "======================================================================"
        //    << endl; 	cout << "Threshold is reached for the first time - goal
        //    leading actions are set" << endl;
...
ShrinkLabelSubsetBisimulation::get_caught_labels_set() const
{
    if (goal_leading == APPROXIMATED_BY_H_MAX ||
        goal_leading == APPROXIMATED_BY_H_MAX_BACKWARD_PRUNED ||
        goal_leading == APPROXIMATED_BY_H_FF) {
...
void ShrinkLabelSubsetBisimulation::handle_option_defaults(
    options::Options& opts)
{
    ShrinkBisimulation::handle_option_defaults(opts);

    /* Peter: Wondered what this is about. Basically, in case of approximation
     * by h_max, we want to consider all facts, even those with a value greater
     * than that of the initial state. Thus, we have to use the highest possible
     * bound.
     */
...
static std::shared_ptr<ShrinkStrategy> _parse(options::OptionParser& parser)
{
    ...
    goal_lead.push_back(
        "shortest_path_relevant_from_threshold_backward_pruning");
    ...
    parser.add_option<bool>(
        "use_all_best_supporters",
        "for approximation by h_max: "
        "should we use all best supporters, or only one for each fact?",
        "true");

    parser.add_option<int>(
        "max_states_before_catching",
