...
void ShrinkStrategy::apply(
    ...
    int target) const
{
    assert(equivalence_relation.size() <= (size_t)target);
    if (equivalence_relation.size() == (size_t)abs.size()) {
        cout << "no size change due to shrink -> not applying EQ" << endl;
        return;
    }
    abs.apply_abstraction(equivalence_relation);
    cout << abs.tag() << "size after shrink " << abs.size() << ", target "
         << target << endl;
    assert(abs.size() <= target);
}

void ShrinkStrategy::add_options_to_parser(options::OptionParser& parser)
{
    // TODO: better documentation what each parameter does
    parser.add_option<int>("max_states", "maximum abstraction size", "-1");
...
void ShrinkStrategy::handle_option_defaults(options::Options& opts)
{
