    ...
    vector<Bucket>& buckets) const
{
    ...
    if (f_start == HIGH) {
        collect_f_h_buckets(
            states_by_f_and_h.rbegin(),
            states_by_f_and_h.rend(),
            h_start,
            buckets);
    } else {
        collect_f_h_buckets(
            states_by_f_and_h.begin(),
            states_by_f_and_h.end(),
            h_start,
            buckets);
    }
    assert(buckets.size() == (size_t)bucket_count);
}
    ...
    vector<Bucket>& buckets) const
{
    ...
    for (AbstractStateRef state = 0; state < abs.size(); ++state) {
        ...
        if (g != infinity && h != infinity) {
            int f = g + h;
            assert(f >= 0 && (size_t)f < states_by_f_and_h.size());
            assert(h >= 0 && (size_t)h < states_by_f_and_h[f].size());
            Bucket& bucket = states_by_f_and_h[f][h];
            if (bucket.empty()) {
                ++bucket_count;
            }
...
std::shared_ptr<ShrinkStrategy> ShrinkFH::create_default(int max_states)
{
    options::Options opts;
    opts.set<int>("max_states", max_states);
    opts.set<int>("max_states_before_merge", max_states);
    opts.set<int>("shrink_f", ShrinkFH::HIGH);
    opts.set<int>("shrink_h", ShrinkFH::LOW);
    return std::make_shared<ShrinkFH>(opts);
}
...
static Plugin<ShrinkStrategy> _plugin("legacy_shrink_fh", _parse);
