...
int FFHeuristic::compute_heuristic(const GlobalState& state)
{
    ...
    for (unsigned i = 0; i < unary_operators.size(); i++) {
        ...
        if (op.unsatisfied_preconditions == 0 && op.effect->cost == op.cost) {
        ...
        for (int val = 0; val < g_variable_domain[var]; val++) {
            if (!m_achievers[prop_id].empty()) {
                ...
                case (ARBITRARY):
                    ...
                    break;
                case (RANDOM):
                    propositions[var][val].reached_by =
                        m_achievers[prop_id]
                                   [m_rng.random(m_achievers[prop_id].size())];
                    break;
                }
            }
            prop_id++;
...
void FFHeuristic::compute_relaxed_plan(
    const GlobalState& state,
    RelaxedPlan& returned_relaxed_plan)
{
    returned_relaxed_plan.clear();
    returned_relaxed_plan.resize(g_operators.size(), false);
    int h_add = compute_add_and_ff(state);
    if (h_add == DEAD_END) return;
