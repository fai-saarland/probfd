#include "legacy/merge_and_shrink/max_regression_operators.h"
...
void MaxRegressionOperators::get_relaxed_operators(vector<bool>& labels) {
        ...
        // giving it the cost of operator + the cost of the proposition in the
effect.
        // We stop when there is no change to the backward queue (that is,
nothing
        // more to add).
        // We return the actions that have unary effect representatives such
that all their
        // preconditions have cost under the bound and the effect is strictly
under the bound.
        ...
        // because of the -1 in the precondition (see drop action in Gripper for
example)
...
//            prop.dump();
    ...
    for (int i = 0; i < goal_propositions.size(); i++) {
        ...
        //TODO: Implement the case of alpha = 0 separately. In this case we need
to collect only minimal cost operators that achieve some goal. if (alpha == 0.0)
{
    ...
    int round = 0;
    // Getting the smallest action cost from the first round, to update the
bound, if too small. int min_action_cost_round_one = numeric_limits<int>::max();
    int prop_count = 0;
...
//    	cout << "============ Round " << round << endl;
        ...
        for (set<Proposition*>::iterator it = curr_reached.begin(); it !=
curr_reached.end(); ++it) { Proposition *prop = *it;
//    		prop->dump();
...
//                cout<< " Fixed cost: " << unary_op->cost << endl;
                ...
                for (int p = 0; p < prec.size(); p++) {
                    if (enqueue_if_necessary(prec[p], unary_op)) {
                        // For the first round only - get the minimal action
cost if (round == 0) min_action_cost_round_one = min(min_action_cost_round_one,
unary_op->cost);
                    }
                }
            }
...
endl;
    ...
    bound = max(bound, min_action_cost_round_one);
        cout << "Using bound " << bound << " for approximating goal leading
labels" << endl;
...
//    cout << "==================== Operators==============" << endl;
    ...
    // Going over the unary operators, marking those that have all preconditions
cost under the bound
    ...
    for (int i = 0; i < unary_operators.size(); i++) {
        UnaryOperator *op = &unary_operators[i];
                ...
                // Changed to get operators from the first layer also in cse
when bound is 0. if (op->effect->cost == -1 ||
                                ((op->effect->cost > 0) and (op->effect->cost >=
bound))) continue;
...
void MaxRegressionOperators::get_relaxed_operators(vector<bool>& labels)
{
    ...
    // achieves a proposition, add its preconditions to the queue,
    // giving it the cost of operator + the cost of the proposition in the
    // effect. We stop when there is no change to the backward queue (that is,
    // nothing more to add). We return the actions that have unary effect
    // representatives such that all their preconditions have cost under the
    // bound and the effect is strictly under the bound. The more strict
    // approach of getting only the actions with all their representatives
    // having such a condition does not seem to work, because of the -1 in the
    // precondition (see drop action in Gripper for example)
    ...
    // The case of alpha = 0 is implemented separately. In this case we need to
    // collect only minimal cost operators that achieve some goal.
    if (alpha == 0.0) {
...
void MaxRegressionOperators::mark_relaxed_operators(
    vector<bool>& labels,
    double bound)
{
    int num_props = 0;
    ...
    // We return the actions that have unary effect representatives such that
    // all their preconditions have cost under the bound and the effect is
    // strictly under the bound. In case the bound is 0 the second inequality is
    // not strict.

    /*// Peter: If we want not necessarily all supporters for each fact, we can
    mark for which
    ...
    // Going over the unary operators, marking those that have all preconditions
    // cost under the bound and effect cost strictly under the bound
    for (size_t i = 0; i < unary_operators.size(); i++) {
        ...
        /*if (!all_best_supporters) {
                // Peter: If we want not necessarily all supporters for each
        fact, we check whether
                // the effect of this unary operator is already achieved based
        on the marked operators.
                // If it is, we ignore it; otherwise we might mark it based on
        the same conditions
        ...
        // Changed to get operators from the first layer also in case when bound
        // is 0.
        if (op->effect->cost == -1 ||
        ...
        if (all_best_supporters) {
            ...
            /*labels[op->operator_no] = true;
            // Peter: mark all effect propositions of the original operator as
            already having a marked supporter
            // Peter: PROBLEM: This sets SOME supporter for each fact within the
            bound, not necessarily one of the BEST supporters.
            // However, if we use the transformation of all action costs to 0
            for deciding unsolvability, this should be OK,
            // as in that case all achieving operators are best supporters.
            // Furthermore, M&S does not allow for conditional effects, so that
            all preconditions of the unary operators created
            // from one original operator must be the same.
            const vector<GlobalEffect> &prepost =
            g_operators[op->operator_no].get_pre_post(); for (int j = 0; j <
            prepost.size(); j++) {
                propositions[prepost[j].var][prepost[j].post].marked_for_label_set_approximation
            = true;
            }*/
    ...
    mark_relevant_operators(
        num_props,
        outgoing_operators,
        reached_propositions,
        labels,
        num_marked_operators);
}
...
{
    // We return the actions that have unary effect representatives such that
    // their effect is in goal, and their cost is the minimal among such.
    ...
    if (all_best_supporters) {
        // Peter: original implementation: take all minimum-cost actions
        // achieving a goal fact
        ...
        // Peter: new implementation: take one achieving minimum-cost action for
        // each goal fact
        /*
         * Original idea: Go over all goal propositions. If some achieving
         * minimum-cost action is already caught, continue with next
         * proposition. Otherwise mark first achieving action
         */
        /*for (int i = 0; i < goal_propositions.size(); i++) {
                // check if at least one supporting action of this proposition
        is already marked
                // and store the index (and cost) of the first cheapest one
                const vector<UnaryOperator*> &triggering_operators =
        goal_propositions[i]->effect_of; bool some_supporter_set = false; int
        min_index = -1; for (int j = 0; j < triggering_operators.size(); j++) {
                ...
                if (!some_supporter_set) {
                        // if not, set supporter to be the first of the cheapest
        operators
                        labels[goal_propositions[i]->effect_of[min_index]->operator_no]
        = true;
                }
        }*/

        /*
         * New idea: For all facts predecessors of goal facts, collect outgoing
         * operators that have a goal fact in their effect. Then first catch all
         * operators for which there is one goal fact predecessor that is
         * reached by regression by only that operator. Finally, catch the
         * operator reaching (by regression) the highest number of goal facts
         * until all goal fact predecessors are reached by regression by all
         * caught operators.
         */
        /*
         * For all goal facts, store the achieving operators with minimum cost
         * and their precondition facts and the inverse, i.e., for such a
         * minimum-cost operator the goal fact predecessors reached by it by
         * regression
         */
            ...
            for (size_t j = 0; j < triggering_operators.size(); j++) {
                ...
                if (op->cost == min_cost) {
                    ...
                    for (size_t k = 0; k < pre.size(); k++) {
                        ...
                        achieved_propositions[op->operator_no].insert(
                            pre[k]->id);
                    }
                }
            }
    ...
    while (prop_count < (int)reached_props.size()) {
        ...
        for (set<Proposition*>::iterator it = curr_reached.begin();
            ...
            for (size_t i = 0; i < triggering_operators.size(); i++) {
                ...
                //                prop->cost) { 	cout << "!!!!! Cost is
                //                too high!" << endl;
                //                }

                unary_op->cost =
                    max(unary_op->cost, unary_op->base_cost + prop->cost);
                ...
                for (size_t p = 0; p < prec.size(); p++) {
                    if (enqueue_if_necessary(prec[p], unary_op)) {
                        ...
                        // cost
                        if (round == 0) {
                            min_action_cost_round_one =
                                min(min_action_cost_round_one, unary_op->cost);
                        }
                    }
                }
            }
