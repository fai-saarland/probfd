...
bool
PerfectHashingFunction::is_within_limits() const
{
    return safe_;
}

std::size_t
PerfectHashingFunction::hash(const std::vector<int>& values) const
{
    assert(safe_);
...
void
PerfectHashingFunction::hash_to_values(
    std::size_t hash,
    const std::vector<int>& variables,
    const std::vector<int>& domains,
...
std::size_t
PerfectHashingFunction::get_max_hash() const
{
    assert(safe_);
    return max_hash_;
}
...
bool
PerfectHashingFunction::set_offset(std::size_t offset)
{
    assert(safe_);
    offset_ = offset;
...
std::size_t
PerfectHashingFunction::get_offset() const
{
    return offset_;
}
