    ...
    struct TransitionSystemData {
        ...
        TransitionSystemData(TransitionSystemData&& other)
            ...
            , label_equivalence_relation(
                  std::move(other.label_equivalence_relation))
...
vector<unique_ptr<Label>> FTSFactory::create_labels()
{
    ...
    for (OperatorProxy op : task_proxy.get_operators()) {
        result.push_back(std::make_unique<Label>(op.get_cost()));
    }
    return result;
}
    ...
    */
    ...
    for (int var_no = 0; var_no < num_variables; ++var_no) {
        ...
        */
        ...
        vector<vector<int>>& label_groups =
            transition_system_data_by_var[var_no].label_groups;
        ...
        for (size_t group_id = 0;
             group_id < existing_transitions_by_group_id.size();
             ++group_id) {
            const vector<Transition>& group_transitions =
                existing_transitions_by_group_id[group_id];
...
FTSFactory::create_transition_systems(const Labels& labels)
{
    ...
    for (int var_no = 0; var_no < num_variables; ++var_no) {
        ...
        ts_data.label_equivalence_relation =
            std::make_unique<LabelEquivalenceRelation>(
                labels,
                ts_data.label_groups);
...
FTSFactory::create_mas_representations() const
{
    ...
    for (int var_no = 0; var_no < num_variables; ++var_no) {
        ...
        result.push_back(
            std::make_unique<MergeAndShrinkRepresentationLeaf>(var_no, range));
    }
    return result;
}
    ...
    utils::LogProxy& log)
{
    ...
    unique_ptr<Labels> labels = std::make_unique<Labels>(create_labels());
