...
const set<int>&
LandmarkCostAssignment::get_achievers(int lmn_status, const Landmark& landmark)
    const
{
    ...
    */
    ...
    for (int lm_id = 0; lm_id < num_cols; ++lm_id) {
        ...
        if (lm_status != lm_reached) {
            const set<int>& achievers = get_achievers(lm_status, landmark);
            if (achievers.empty()) return numeric_limits<double>::max();
            for (int op_id : achievers) {
    ...
    for (const lp::LPConstraint& constraint : lp_constraints) {
        if (!constraint.empty()) lp.get_constraints().push_back(constraint);
    }
