    ...
    bool obedient_orders)
{
    ...
    */
    ...
    for (auto& node_p : lm_graph->get_nodes()) {
        ...
        if (landmark.is_true_in_state(initial_state)) return;
        ...
        if (!obedient_orders && landmark.is_true_in_goal) {
            ...
            // Insert reasonable orders between those members of "interesting
            // nodes" that interfere with node_p.
    ...
    const Landmark& landmark_b) const
{
    ...
    for (const FactPair& lm_fact_b : landmark_b.facts) {
        ...
        for (const FactPair& lm_fact_a : landmark_a.facts) {
            ...
            // achieved through a sequence of operators successively adding the
            // parts of a
            if (landmark_a.conjunctive) continue;

            unordered_map<int, int> shared_eff;
            bool init = true;
            const vector<int>& op_or_axiom_ids =
                get_operators_including_eff(lm_fact_a);
            ...
            for (int op_or_axiom_id : op_or_axiom_ids) {
                ...
                // it is an unconditional effect or a conditional effect that is
                // sure to happen. (Such "trivial" conditions can arise due to
                // our translator, e.g. in Schedule. There, the same effect is
                // conditioned on a disjunction of conditions of which one will
                ...
                // Intersect effects of this operator with those of previous
                // operators
                if (init)
    ...
    bool use_reasonable)
{
    ...
    for (const auto& p : node.parents) {
        ...
        const EdgeType& edge = p.second;
        if (edge >= EdgeType::NATURAL ||
            (use_reasonable && edge == EdgeType::REASONABLE))
...
{
    /* Test whether the condition of a conditional effect is trivial, i.e.
     always true. We test for the simple case that the same effect proposition
     is triggered by a set of conditions of which one will always be true. This
     is e.g. the case in Schedule, where the effect (forall (?oldpaint - colour)
     (when (painted ?x ?oldpaint)
     (not (painted ?x ?oldpaint))))
     is translated by the translator to: if oldpaint == blue, then not painted
     ?x, and if oldpaint == red, then not painted ?x etc. If conditional effects
     are found that are always true, they are returned in "eff".
     */
    ...
    // (nogood_effect_vars)
    // - a mapping from cond. effect propositions to all the conditions that
    // they appear with
    set<int> effect_vars;
    ...
    for (EffectProxy effect : effects) {
        ...
        if (effect_vars.find(var_id) != effect_vars.end()) {
            ...
            assert(
                effect_conditions_by_variable.find(var_id) !=
    ...
    // For all those effect propositions whose variables do not take on
    // different values...
    for (const auto& effect_conditions : effect_conditions_by_variable) {
        ...
        for (const FactPair& cond : effect_conditions.second.second) {
            if (unique_conds.find(cond.var) != unique_conds.end()) {
                ...
                set<int>& the_set =
                    unique_conds.emplace(cond.var, set<int>()).first->second;
                the_set.insert(cond.value);
            }
        }
        // Check for each condition variable whether the number of values it
        // takes on is equal to the domain of that variable...
        ...
        for (auto& unique_cond : unique_conds) {
            ...
            int num_values_of_variable =
                variables[unique_cond.first].get_domain_size();
            ...
            // values except the effect value
                ...
                unique_cond.first == effect_conditions.first &&
                num_values_for_cond == num_values_of_variable - 1) {
                // Number of different values is correct, now ensure that the
                // effect value was the one missing
            ...
            // If one of the condition variables does not fulfill the criteria,
            // the effect is not certain to happen
            if (!is_surely_reached_by_var) is_always_reached = false;
        }
        if (is_always_reached)
            eff.insert(FactPair(
                effect_conditions.first,
                effect_conditions.second.first));
    }
    return eff.empty();
}
