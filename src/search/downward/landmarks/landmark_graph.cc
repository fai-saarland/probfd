...
{
    int total = 0;
    for (auto& node : nodes) total += node->children.size();
    return total;
}
...
LandmarkNode* LandmarkGraph::get_node(const FactPair& fact) const
{
    ...
    if (it != simple_landmarks_to_nodes.end())
        ...
        if (it2 != disjunctive_landmarks_to_nodes.end()) node_p = it2->second;
    }
    return node_p;
}
...
LandmarkNode&
LandmarkGraph::get_disjunctive_landmark(const FactPair& fact) const
{
    ...
    const set<FactPair>& lm) const
{
    ...
    for (const FactPair& lm_fact : lm) {
        if (contains_disjunctive_landmark(lm_fact)) return true;
    }
    return false;
}
...
void LandmarkGraph::remove_node(LandmarkNode* node)
{
    ...
    auto it = find_if(
        nodes.begin(),
        nodes.end(),
        [&node](unique_ptr<LandmarkNode>& n) { return n.get() == node; });
    assert(it != nodes.end());
    nodes.erase(it);
}
