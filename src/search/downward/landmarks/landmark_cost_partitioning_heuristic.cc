    ...
    const plugins::Options& opts)
{
    if (cost_partitioning_strategy == CostPartitioningStrategy::OPTIMAL) {
        ...
        cost_partitioning_strategy == CostPartitioningStrategy::UNIFORM) {
        lm_cost_assignment =
...
class LandmarkCostPartitioningHeuristicFeature
    : public plugins::
          TypedFeature<Evaluator, LandmarkCostPartitioningHeuristic> {
public:
        ...
        : TypedFeature("landmark_cost_partitioning")
    {
        ...
        document_note(
            ...
            "with LP support. See LPBuildInstructions.");
        document_note(
            "Preferred operators",
