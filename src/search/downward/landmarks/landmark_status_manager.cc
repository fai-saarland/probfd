    ...
    utils::LogProxy& log)
{
    ...
    // This is necessary since the default is "true for all" (see comment
    // above).
    reached.reset();
    ...
    if (log.is_at_least_normal()) {
        log << inserted << " initial landmarks, " << num_goal_lms
            << " goal landmarks" << endl;
    }
}
...
bool LandmarkStatusManager::landmark_needed_again(int id, const State& state)
{
    LandmarkNode* node = lm_graph.get_node(id);
