#include "downward/search_engines/enforced_hill_climbing_search.h"
    ...
    if (!use_preferred ||
        ...
        */
        ...
        vector<shared_ptr<Evaluator>> evals = {
            g_evaluator,
            make_shared<PrefEval>(pref_evaluator_options)};
        plugins::Options options;
    ...
    , last_num_expanded(-1)
{
    ...
    open_list =
        create_ehc_open_list_factory(opts, use_preferred, preferred_usage)
...
class EnforcedHillClimbingSearchFeature
    : public plugins::TypedFeature<SearchEngine, EnforcedHillClimbingSearch> {
public:
