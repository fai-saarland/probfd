...
EquivalenceRelation::EquivalenceRelation(int n, const list<Block>& blocks_)
    ...
    , blocks(blocks_)
{
    for (BlockListIter it_block = blocks.begin(); it_block != blocks.end();
         ++it_block) {
        Block& block = *it_block;
        for (ElementListIter it_element = block.begin();
             it_element != block.end();
             ++it_element) {
...
{
    for (BlockListConstIter it = other.blocks.begin(); it != other.blocks.end();
         ++it) {
        refine(*it);
    }
}
...
/*
  ...
  The sparse definition need special attention since there can be elements x \in
  X that are not specified in our relation. All unspecified elements are assumed
  to belong to a block U. Splitting U along X yields (U \cap X) and (U \setminus
  X). The block (U \setminus X) contains all elements that are neither specified
...
void EquivalenceRelation::refine(
    ElementListConstIter block_x_begin,
    ElementListConstIter block_x_end)
{
    ...
    // For every x \in X:
    for (ElementListConstIter it_x = block_x_begin; it_x != block_x_end;
         ++it_x) {
        int x = *it_x;
        ...
        if (it_pos != element_positions.end()) {
            ...
            // Add x to (B \cap X).
            ElementListIter it_element =
                block_B.it_intersection_block->insert(x);
            // Update the stored position of x.
            pos.first = block_B.it_intersection_block;
            pos.second = it_element;
        } else {
            // x is unspecified in our relation, so it belongs in block (U \cap
