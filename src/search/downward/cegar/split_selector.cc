#include "downward/cegar/split_selector.h"
...
int SplitSelector::get_min_hadd_value(int var_id, const vector<int>& values)
    const
{
...
int SplitSelector::get_max_hadd_value(int var_id, const vector<int>& values)
    const
{
    int max_hadd = -1;
    for (int value : values) {
...
double
SplitSelector::rate_split(const AbstractState& state, const Split& split) const
{
    ...
    case PickSplit::MIN_REFINED:
        ...
        break;
    case PickSplit::MAX_REFINED: rating = get_refinedness(state, var_id); break;
