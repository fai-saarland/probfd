...
_intersect(const unordered_map<int, int>& a, const unordered_map<int, int>& b)
{
...
{
    if (op_or_axiom_id < 0) {
        return task.get_axioms()[-op_or_axiom_id - 1];
    } else {
        return task.get_operators()[op_or_axiom_id];
    }
}
    ...
    utils::LogProxy& log)
{
    if (log.is_at_least_debug()) {
        ...
        for (FactPair fact : landmark.facts) {
            ...
            first = false;
            VariableProxy var = task.get_variables()[fact.var];
            cout << var.get_fact(fact.value).get_name();
        }
        cout << "\"";
        if (landmark.is_true_in_state(task.get_initial_state())) {
            cout << ", style=bold";
        }
...
static void dump_edge(int from, int to, EdgeType edge, utils::LogProxy& log)
{
    if (log.is_at_least_debug()) {
        ...
        switch (edge) {
        case EdgeType::NECESSARY: cout << "\"nec\""; break;
        case EdgeType::GREEDY_NECESSARY: cout << "\"gn\""; break;
        case EdgeType::NATURAL: cout << "\"n\""; break;
        case EdgeType::REASONABLE: cout << "\"r\", style=dashed"; break;
        }
        cout << "];\n";
    }
}
