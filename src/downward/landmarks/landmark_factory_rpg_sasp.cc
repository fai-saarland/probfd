...
{
    if (pre != post) dtg_successors[var_id][pre].insert(post);
}
    ...
    */
    ...
    for (VariableProxy var : variables) {
        ...
        for (int value = 0; value < num_values; ++value) {
            string predicate =
                get_predicate_for_fact(variables, var.get_id(), value);
            int disj_class;
            if (predicate.empty()) {
                disj_class = -1;
            } else {
    ...
    const Landmark& landmark)
{
    ...
    */
    ...
    for (size_t i = 0; i < op_or_axiom_ids.size(); ++i) {
        AxiomOrOperatorProxy op =
            get_operator_or_axiom(task, op_or_axiom_ids[i]);
        if (possibly_reaches_lm(op, reached, landmark)) {
            ++num_ops;
            unordered_map<int, int> next_pre;
            get_greedy_preconditions_for_lm(task, landmark, op, next_pre);
    ...
    Exploration& exploration)
{
    ...
    build_dtg_successors(*task);
    build_disjunction_classes(*task);
    ...
    while (!open_landmarks.empty()) {
        ...
        if (!landmark.is_true_in_state(initial_state)) {
            ...
            */
            unordered_map<int, int> shared_pre;
            compute_shared_preconditions(*task, shared_pre, reached, landmark);
            /*
    ...
    LandmarkNode* lmp)
{
    ...
    */
    ...
    if (landmark.disjunctive) return;
    ...
    const unordered_set<int>& exclude)
{
    /*
      ...
      a path from the initial state value to the LM value, without passing
      through any value in "exclude". If not, that means that one of the values
      in "exclude" is crucial for achieving the landmark (i.e. is on every path
      to the LM).
    */
    ...
    */
    for (VariableProxy var : variables)
        for (int value = 0; value < var.get_domain_size(); ++value) {
            if (reached[var.get_id()][value]) continue;
            const FactPair fact(var.get_id(), value);

            bool insert = true;
            for (const FactPair& lm_fact : lm_node->get_landmark().facts) {
                if (fact != lm_fact) {
                    // Make sure there is no operator that reaches both lm and
                    // (var, value) at the same time
                    ...
                    const vector<int>& reach_lm =
                        get_operators_including_eff(lm_fact);
                    for (size_t j = 0;
                         j < reach_fact.size() && intersection_empty;
                         ++j)
                        for (size_t k = 0;
                             k < reach_lm.size() && intersection_empty;
                             ++k)
            ...
            if (insert) forward_orders[lm_node].insert(fact);
        }
}
