    ...
    vector<FluentSet>& subsets)
{
    ...
    for (int i = 0; i < variables[current_var].get_domain_size(); ++i) {
        ...
        for (const FactPair& current_fact : current) {
            if (!interesting(mutexes, current_var_fact, current_fact)) {
                use_var = false;
                break;
            }
        }
    ...
    const FluentSet& superset)
{
    ...
    for (const FactPair& fluent : current) {
        if (!interesting(mutexes, superset[current_var_index], fluent)) {
            use_var = false;
            break;
        }
    }
    ...
    const FluentSet& superset2)
{
    ...
    if (ss1_var_index != sup1_size &&
        ...
        for (const FactPair& fluent : current) {
            if (!interesting(mutexes, superset1[ss1_var_index], fluent)) {
                use_var = false;
                break;
            }
        }
        ...
        for (const FactPair& fluent : current) {
            if (!interesting(mutexes, superset2[ss2_var_index], fluent)) {
                use_var = false;
                break;
            }
        }
...
{
    FluentSet c;
    get_m_sets_(task_proxy, mutexes, m, 0, 0, c, subsets);
}
...
{
    FluentSet c;
    get_m_sets_of_set(task_proxy, mutexes, m, 0, 0, c, subsets, superset);
}
    ...
    const FactPair& fluent) const
{
    if (log.is_at_least_verbose()) {
        ...
        log << fact.get_name() << " (" << var.get_name() << "("
...
static FluentSet
get_operator_postcondition(int num_vars, const OperatorProxy& op)
{
    ...
    if (log.is_at_least_verbose()) {
        ...
        for (size_t i = 0; i < op.cond_noops.size(); ++i) {
            ...
            for (int pm_fluent; (pm_fluent = op.cond_noops[i][j]) != -1; ++j) {
                ...
                for (size_t k = 0; k < h_m_table_[pm_fluent].fluents.size();
                     ++k) {
...
bool LandmarkFactoryHM::possible_noop_set(
    const MutexInformation& mutexes,
    const FluentSet& fs1,
    const FluentSet& fs2)
{
    ...
    for (const FactPair& fluent1 : fs1) {
        for (const FactPair& fluent2 : fs2) {
            if (mutexes.are_facts_mutex(fluent1, fluent2)) return false;
        }
    }
...
{
    FluentSet pc, eff;
    vector<FluentSet> pc_subsets, eff_subsets, noop_pc_subsets,
        noop_eff_subsets;
    ...
    for (OperatorProxy op : operators) {
        ...
        get_m_sets(task_proxy, mutexes, m_, pc_subsets, pc);
        ...
        get_m_sets(task_proxy, mutexes, m_, eff_subsets, eff);
        ...
        while (static_cast<int>(it->first.size()) < m_ &&
               it != set_indices_.end()) {
            if (possible_noop_set(mutexes, eff, it->first)) {
                ...
                // get the subsets that have >= 1 element in the pc (unless pc
                ...
                this_cond_noop.reserve(
                    noop_pc_subsets.size() + noop_eff_subsets.size() + 1);

                unsat_pc_count_[op.get_id()].second.push_back(
                    noop_pc_subsets.size());
                ...
                for (size_t j = 0; j < noop_pc_subsets.size(); ++j) {
                    ...
                    h_m_table_[set_index].pc_for.emplace_back(
                        op.get_id(),
                        noop_index);
                }
...
bool LandmarkFactoryHM::interesting(
    const MutexInformation& mutexes,
    ...
    const MutexInformation& mutexes)
{
    ...
    get_m_sets(task_proxy, mutexes, m_, msets);
    ...
    build_pm_ops(task_proxy, mutexes);
}
    ...
    const MutexInformation& mutexes)
{
    ...
    calc_achievers(task_proxy, mutexes);
}
...
void LandmarkFactoryHM::discard_conjunctive_landmarks()
{
    if (lm_graph->get_num_conjunctive_landmarks() > 0) {
        ...
        lm_graph->remove_node_if([](const LandmarkNode& node) {
    ...
    const MutexInformation& mutexes)
{
    ...
    for (auto& lm_node : lm_graph->get_nodes()) {
        ...
        for (int op_id : candidates) {
            FluentSet post =
                get_operator_postcondition(variables.size(), operators[op_id]);
            ...
            for (j = 0; j < landmark.facts.size(); ++j) {
                ...
                for (const FactPair& fluent : post) {
                    if (mutexes.are_facts_mutex(fluent, lm_fact)) {
                        is_mutex = true;
                        break;
                    }
                }
...
void LandmarkFactoryHM::propagate_pm_fact(
    int factindex,
    ...
    TriggerSet& trigger)
{
    ...
    for (const FactPair& info : h_m_table_[factindex].pc_for) {
        ...
        // a pc for a conditional noop
            ...
            // if associated action is applicable, and effect has become
            // applicable (if associated action is not applicable, all noops
            // will be used when it first does)
            if ((unsat_pc_count_[info.var].first == 0) &&
    ...
    const MutexInformation& mutexes)
{
    ...
    while (!current_trigger.empty()) {
        for (op_it = current_trigger.begin(); op_it != current_trigger.end();
            ...
            // in the set of landmarks for each fact, the fact itself is not
            // stored (only landmarks preceding it)
            ...
            if (op_it->second.empty()) {
                for (size_t i = 0; i < action.cond_noops.size(); ++i) {
                    // actions pcs are satisfied, but cond. effects may still
                    // have unsatisfied pcs
    ...
    for (; i < pc_eff_pair.size(); ++i) {
        ...
        if (h_m_table_[pm_fluent].level != -1) {
            ...
            if (!contains(cn_landmarks, pm_fluent)) {
                ...
                if (use_orders) {
                    intersect_with(
                        h_m_table_[pm_fluent].necessary,
                        cn_necessary);
                }
            }
...
void LandmarkFactoryHM::generate_landmarks(const shared_ptr<AbstractTask>& task)
{
    ...
    compute_h_m_landmarks(task_proxy, *mutexes);
    ...
    VariablesProxy variables = task_proxy.get_variables();
    get_m_sets(task_proxy, *mutexes, m_, goal_subsets, goals);
    list<int> all_lms;
    for (const FluentSet& goal_subset : goal_subsets) {
        ...
        if (h_m_table_[set_index].level == -1) {
            if (log.is_at_least_verbose()) {
                log << endl
                    << endl
                    << "Subset of goal not reachable !!." << endl
                    << endl
                    << endl;
                log << "Subset is: ";
    ...
    postprocess(task_proxy, *mutexes);
}
