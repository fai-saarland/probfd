...
static vector<pair<LandmarkNode*, vector<LandmarkNode*>>>
...
LandmarkStatusManager::LandmarkStatusManager(
    ...
    : lm_graph(graph)
    , goal_landmarks(
          progress_goals ? get_goal_landmarks(graph) : vector<LandmarkNode*>{})
...
ConstBitsetView
LandmarkStatusManager::get_past_landmarks(const State& state) const
{
    return past_landmarks[state];
}

ConstBitsetView
LandmarkStatusManager::get_future_landmarks(const State& state) const
{
    return future_landmarks[state];
}
...
void LandmarkStatusManager::progress(
    const State& parent_ancestor_state,
    OperatorID,
    ...
    for (auto& node : lm_graph.get_nodes()) {
        ...
        if (parent_future.test(id)) {
            if (!lm.is_true_in_state(ancestor_state)) {
                ...
                */
                future.set(id);
                if (!parent_past.test(id)) { past.reset(id); }
