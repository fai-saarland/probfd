    ...
    : task(task)
{
    ...
    for (int var = 0; var < var_count; ++var) {
        for (auto succ_var : cg.get_pre_to_eff(var)) {
            ...
            if (succ_var > var) depends_on[succ_var].push_back(var);
        }
    }
    ...
    int max_cache_size) const
{
    ...
    */
    ...
    for (int depend_var_id : depends_on) {
        ...
        */
        if (cache[depend_var_id].empty()) return -1;
...
int CGCache::get_index(int var, const State& state, int from_val, int to_val)
    const
{
