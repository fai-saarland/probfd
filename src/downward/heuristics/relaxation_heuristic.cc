...
const Proposition*
RelaxationHeuristic::get_proposition(int var, int value) const
{
    ...
    */
    ...
    for (size_t op_no = 0; op_no < unary_operators.size(); ++op_no) {
        ...
        */
        ...
        auto inserted =
            unary_operator_index.insert(make_pair(std::move(key), value));
        if (!inserted.second) {
    ...
    */
    auto is_dominated = [&](const UnaryOperator& op) {
        ...
        */
        ...
        // We subtract "- 1" to generate all *strict* subsets of
        // precondition.
