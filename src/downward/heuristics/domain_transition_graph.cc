...
void DTGFactory::allocate_graphs_and_nodes(DTGs& dtgs)
{
    ...
    for (VariableProxy var : variables) {
        ...
        dtgs[var_id] = std::make_unique<DomainTransitionGraph>(var_id, range);
    }
}
    ...
    unsigned int first_local_var)
{
    ...
    if (loc_to_glob.size() > first_local_var)
        loc_to_glob.erase(
            loc_to_glob.begin() + first_local_var,
            loc_to_glob.end());
}

ValueTransition*
DTGFactory::get_transition(int origin, int target, DomainTransitionGraph* dtg)
{
    ...
    for (auto& label : labels) {
        ...
        for (EffectProxy eff : op.get_effects()) {
            ...
            if (includes(
                    precond_pairs.begin(),
                    precond_pairs.end(),
                    triggercond_pairs.begin(),
                    triggercond_pairs.end())) {
...
void DTGFactory::simplify_labels(vector<ValueTransitionLabel>& labels)
{
    ...
    for (auto& entry : label_index) {
        ...
        if (!match) labels.push_back(old_labels[label_no]);
    }
}
