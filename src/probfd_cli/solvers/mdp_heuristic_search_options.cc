    ...
    const Options& options)
{
    return std::tuple_cat(
        std::make_tuple(
            ...
            options.get<bool>(context, "dual_bounds"),
...
get_mdp_hs_args_from_options(const Context& context, const Options& options)
{
    if constexpr (Fret) {
        return std::tuple_cat(
            std::make_tuple(options.get<bool>(context, "fret_on_policy")),
            get_mdp_hs_base_args_from_options<Bisimulation, Fret>(
                context,
                options));
    } else {
        return get_mdp_hs_base_args_from_options<Bisimulation, Fret>(
            context,
            options);
    }
}
