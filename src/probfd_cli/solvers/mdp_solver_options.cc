...
MDPSolverArgs get_base_solver_args_from_options(
    ...
    return std::tuple_cat(
        std::make_tuple(
            options.get<std::shared_ptr<StatisticalMDPAlgorithmFactory>>(
                context,
                "algorithm"),
            options.get<std::shared_ptr<TaskStateSpaceFactory>>(
                context,
                "state_space"),
            options.get<std::shared_ptr<TaskHeuristicFactory>>(
                context,
                "heuristic")),
        get_log_arguments_from_options(context, options),
        std::make_tuple(
            options.get<std::string>(context, "policy_file"),
            options.get<bool>(context, "print_fact_names"),
            options.contains("report_epsilon")
                ? std::optional<value_t>(
                      options.get<value_t>(context, "report_epsilon"))
                : std::nullopt,
            options.get<bool>(context, "report_enabled")));
}

MDPSolverNoAlgorithmArgs get_base_solver_args_no_algorithm_from_options(
    const Context& context,
    const Options& options)
{
    return std::tuple_cat(
        ...
        get_log_arguments_from_options(context, options),
        std::make_tuple(
            options.get<bool>(context, "discard_policy")
                ? std::nullopt
                : std::optional(
                      options.get<std::string>(context, "policy_file")),
            options.get<bool>(context, "print_fact_names"),
