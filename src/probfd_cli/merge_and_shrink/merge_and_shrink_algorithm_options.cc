#include "probfd_cli/merge_and_shrink/merge_and_shrink_algorithm_options.h"
    ...
    return tuple_cat(
        make_tuple(
            ...
            opts.get<shared_ptr<LabelReduction>>(
                ...
                "label_reduction",
                nullptr),
            opts.get<shared_ptr<PruneStrategy>>(context, "prune_strategy")),
        get_transition_system_size_limit_arguments_from_options(context, opts),
        make_tuple(opts.get<downward::utils::Duration>(
            context,
            "main_loop_max_time")));
}
