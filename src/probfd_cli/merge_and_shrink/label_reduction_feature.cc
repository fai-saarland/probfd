...
public:
    ...
    void dump_options(utils::LogProxy& log) const override
    {
        if (log.is_at_least_normal()) {
            ...
            log.println(
                "Before merging: {}",
                before_merging ? "enabled" : "disabled");
            log.println(
                "Before shrinking: {}",
                before_shrinking ? "enabled" : "disabled");
            log.print("Method: ");
            switch (method) {
            ...
            log.println();
            if (method == LabelReductionMethod::ALL_TRANSITION_SYSTEMS ||
...
public:
    ...
    create_component(const Options& options, const Context& context)
        const override
    {
        const bool lr_before_shrinking =
            options.get<bool>(context, "before_shrinking");
        const bool lr_before_merging =
            options.get<bool>(context, "before_merging");
        ...
        return make_shared_from_arg_tuples<DefaultLabelReductionFactory>(
            ...
            options.get<LabelReductionSystemOrder>(context, "system_order"),
