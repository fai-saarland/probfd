...
public:
    ...
    create_component(const Options& opts, const Context& context) const override
    {
        return make_shared_from_arg_tuples<MergeStrategyFactorySCCs>(
            opts.get<OrderOfSCCs>(context, "order_of_sccs"),
            opts.get<shared_ptr<MergeSelector>>(context, "merge_selector"),
            get_merge_strategy_arguments_from_options(context, opts));
    }
};
} // namespace
