#include "downward_cli/merge_and_shrink/merge_and_shrink_algorithm_options.h"
    ...
    const Options& opts)
{
    return tuple_cat(
        make_tuple(
            opts.get<shared_ptr<MergeStrategyFactory>>(
                context,
                "merge_strategy"),
            ...
            opts.get<shared_ptr<LabelReduction>>(
                ...
                "label_reduction",
                nullptr),
            opts.get<bool>(context, "prune_unreachable_states"),
            opts.get<bool>(context, "prune_irrelevant_states")),
        get_transition_system_size_limit_arguments_from_options(context, opts),
        make_tuple(opts.get<utils::Duration>(context, "main_loop_max_time")));
}
    ...
    const Options& opts)
{
    return make_tuple(
        ...
        opts.get<int>(context, "max_states_before_merge"),
        opts.get<int>(context, "threshold_before_merge"));
}
