...
public:
        ...
        const override
    {
        ...
        return make_shared<IteratedSearchFactory>(
            options.get<OperatorCost>(context, "cost_type"),
            options.get<int>(context, "bound"),
            options.get<utils::Duration>(context, "max_time"),
            options.get_unparsed_config(),
            options.get<utils::Verbosity>(context, "verbosity"),
            options.get_list<
                std::shared_ptr<TaskDependentFactory<SearchAlgorithm>>>(
                context,
                "algorithm_configs"),
            options.get<bool>(context, "pass_bound"),
            options.get<bool>(context, "repeat_last"),
            options.get<bool>(context, "continue_on_fail"),
            options.get<bool>(context, "continue_on_solve"));
    }
};
} // namespace
