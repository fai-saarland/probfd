...
{
    return make_tuple(opts.get<shared_ptr<PruningMethod>>(context, "pruning"));
}
    ...
    const plugins::Options& opts)
{
    return tuple_cat(
        make_tuple(
            opts.get<bool>(context, "randomize_successors"),
            opts.get<bool>(context, "preferred_successors_first")),
