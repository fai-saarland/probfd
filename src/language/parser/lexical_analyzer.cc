...
{
    return regex("^\\s*(" + s + ")\\s*", regex_constants::ECMAScript);
}
...
static vector<pair<TokenType, regex>> construct_token_type_expressions()
{
    ...
    for (const auto& [ttype, str] : token_type_str_expression) {
        token_type_expression.emplace_back(ttype, build_regex(str));
    }
...
static string highlight_position(const string& text, string::const_iterator pos)
{
    ...
    const auto it =
        std::ranges::find_if(message | std::views::reverse, [](int ch) {
            return !isspace(ch);
        });
    message.erase(it.base(), message.end());
    return message;
}
