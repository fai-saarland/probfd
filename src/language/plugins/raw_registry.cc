...
FeatureTypes RawRegistry::collect_types(vector<string>& errors) const
{
    ...
    for (const auto& names : type_to_names | views::values) {
        if (names.size() > 1) {
            errors.push_back(
                std::format(
                    "Multiple plugins are defined for the same type: '{:s}'.",
                    std::views::join_with(names, ", ")));
        }
    }
...
void RawRegistry::validate_category_names(vector<string>& errors) const
{
    ...
    for (const auto& [category_name, class_names] :
        ...
        if (class_names.size() > 1) {
            errors.push_back(
                std::format(
                    "Multiple CategoryPlugins have the name '{}': {:s}.",
                    category_name,
                    std::views::join_with(class_names, ", ")));
        }
    }
        ...
        if (category_names.size() > 1) {
            errors.push_back(
                std::format(
                    ...
                    "{:s}.",
                    class_name,
                    std::views::join_with(category_names, ", ")));
        }
    }
}
    ...
    if (!errors.empty()) {
        ...
        throw std::runtime_error(
            std::format(
                ...
                std::views::join_with(std::move(errors), "\n")));
    }
