...
void add_search_subcommand(argparse::ArgumentParser& arg_parser)
{
    ...
    search_parser.add_argument("algorithm")
        .help(
            ...
            "\n"
            "EXPRESSION := LET_EXPRESSION | LIST_EXPRESSION | FUNCTION_CALL | IDENTIFIER | BOOLEAN | STRING | INTEGER | FLOAT\n"
            "LET_EXPRESSION := 'let' VARIABLE_DEFINITION [ , VARIABLE_DEFINITION ]* 'in' EXPRESSION\n"
            "VARIABLE_DEFINITION := EXPRESSION 'as' IDENTIFIER\n"
            "IDENTIFIER := LETTER [ LETTER | DIGIT ]* (except 'infinity', 'in', 'as', 'let', 'true', 'false')\n"
            "LIST_EXPRESSION := '[' [ EXPRESSION_LIST ] ']'\n"
            "EXPRESSION_LIST := EXPRESSION [ , EXPRESSION ]*\n"
            "FUNCTION_CALL := IDENTIFIER '(' [ POSITIONAL_ARGUMENTS ] [ KEYWORD_ARGUMENTS ] ')'\n"
            "POSITIONAL_ARGUMENTS := EXPRESSION_LIST\n"
            "KEYWORD_ARGUMENTS := IDENTIFIER '=' EXPRESSION [ , IDENTIFIER '=' EXPRESSION ]*\n"
            "BOOLEAN := 'true' | 'false'\n"
            "STRING := '\"' <any non-control character except '\"'> '\"'\n"
            "INTEGER := [ '+' | '-' ] POSITIVE_INTEGER | 'infinity' \n"
            "POSITIVE_INTEGER := '0' | [ 1-9 ] [ 0-9 ]*\n"
            "FLOAT := INTEGER [ '.' POSITIVE_INTEGER ] [ 'e' INTEGER ] [ 'k' | 'm' | 'g' ]\n"
            "\n"
