...
def parse_effects(alist, result, type_dict, predicate_dict):
    """
    Parse PDDL effects, do some simple normalization, and translate them into pddl.Effect
    objects. All pddl.Effect objects are in turn wrapped in pddl.ProbabilisticEffect objects.
    Non-probabilistic actions will have a single pddl.ProbabilisticEffect consisting of a single
    outcome.
    """
...
def add_effect(tmp_effect, result):
    if isinstance(tmp_effect, pddl.ProbabilisticEffect):
        return add_probabilistic_effect(tmp_effect, result)

    det_effs = []
    reward = None
    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        for eff in tmp_effect.effects:
            if isinstance(eff, pddl.ProbabilisticEffect):
                add_probabilistic_effect(eff, result)
            else:
                reward = add_non_prob_effect(eff, det_effs) or reward
    else:
        reward = add_non_prob_effect(tmp_effect, det_effs)

    if len(det_effs) > 0 or reward != None:
        result.append(
            fo_action.ProbabilisticEffect(
                [fo_action.ProbabilisticOutcome(1, det_effs, reward)]
            )
        )


def add_probabilistic_effect(tmp_effect, result):
    assert isinstance(tmp_effect, pddl.ProbabilisticEffect)
    outcomes = []
    for eff in tmp_effect.effects:
        prob_effs = []
        reward = add_non_prob_effect(eff.effect, prob_effs)
        outcomes.append(fo_action.ProbabilisticOutcome(eff.prob, prob_effs, reward))
    result.append(fo_action.ProbabilisticEffect(outcomes))


def add_non_prob_effect(tmp_effect, result):
    ...
    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        ...
        for effect in tmp_effect.effects:
            reward = add_non_prob_effect(effect, result) or reward
        return reward
    else:
        parameters = []
        ...
        if isinstance(tmp_effect, pddl.UniversalEffect):
            ...
            assert isinstance(tmp_effect.effect, pddl.SimpleEffect),\
                f"effects not normalized; expected SimpleEffect but got {type(tmp_effect.effect)} {repr(tmp_effect.effect)}"
            effect = tmp_effect.effect.effect
        elif isinstance(tmp_effect, pddl.RewardEffect):
            return tmp_effect.effect
        else:
        ...
        new_effect = fo_action.Effect(parameters, condition, effect)
        contradiction = fo_action.Effect(parameters, condition, effect.negate())
...
def parse_effect(alist, type_dict, predicate_dict):
    ...
    elif tag == "increase" or tag == "decrease":
        assert len(alist) == 3
        if alist[1] == ["total-cost"]:
            return pddl.RewardEffect(parse_cost_assignment(alist))
        elif alist[1] == ["reward"]:
            return pddl.RewardEffect(parse_reward_assignment(alist))
        else:
            assert False, "unsupported f_expression"
    elif tag == "probabilistic":
        assert (len(alist) - 1) % 2 == 0, (
            "Each probabilistic outcome must have an " "associated probability"
        )
        return pddl.ProbabilisticEffect(
            [
                pddl.ProbabilityWeightedEffect(
                    alist[i], parse_effect(alist[i + 1], type_dict, predicate_dict)
                )
...
def parse_reward_assignment(alist):
    assert len(alist) == 3
    op = alist[0]
    head = parse_expression(alist[1])
    exp = parse_expression(alist[2])
...
def parse_action(alist, type_dict, predicate_dict):
    ...
    if effect_list:
        try:
            parse_effects(effect_list, eff, type_dict, predicate_dict)
        except ValueError as e:
            raise SystemExit("Error in Action %s\nReason: %s." % (name, e))
    for rest in iterator:
        assert False, rest
    if eff:
        return pddl.Action(name, parameters, len(parameters), precondition, eff)
    else:
        return None
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    for fact in init[1:]:
        if fact[0] == "=":
            try:
                if fact[1] == "total-cost":
                    assignment = parse_cost_assignment(fact)
                else:
                    assignment = parse_reward_assignment(fact)
            except ValueError as e:
                raise SystemExit(
    ...
    for entry in iterator:
        if entry[0] == ":metric":
            if (entry[1] == "minimize" and entry[2][0] == "total-cost") or (
                entry[1] == "maximize" and entry[2] == ["reward"]
            ):
                use_metric = True
            else:
