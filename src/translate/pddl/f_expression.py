

class FunctionalExpression(object):
    def __init__(self, parts):
        self.parts = tuple(parts)

    def dump(self, indent="  "):
        print("%s%s" % (indent, self))
        for part in self.parts:
            part.dump(indent + "  ")

    def __str__(self):
        return self.__class__.__name__

    def instantiate(self, var_mapping, init_facts):
        raise ValueError("Cannot instantiate condition: not normalized")


class NumericConstant(FunctionalExpression):
    parts = ()

    def __init__(self, value):
        if value != int(value):
            raise ValueError("Fractional numbers are not supported")
        self.value = int(value)

    def __eq__(self, other):
        return self.__class__ == other.__class__ and self.value == other.value

    def __str__(self):
        return "%s %s" % (self.__class__.__name__, self.value)

    def __add__(self, other):
        return NumericConstant(self.value + other.value)

    def __sub__(self, other):
        return NumericConstant(self.value - other.value)

    def __neg__(self):
        return NumericConstant(-self.value)

    def instantiate(self, var_mapping, init_facts):
        return self


class NegateF(FunctionalExpression):
    def __init__(self, expr: FunctionalExpression):
        self.expr = expr

    def __eq__(self, other):
        return isinstance(other, NegateF) and self.expr == other.expr

    def __str__(self):
        return f"-{self.expr}"

    def instantiate(self, var_mapping, init_facts):
        return self.expr.instantiate(var_mapping, init_facts).__neg__()


class SumF(FunctionalExpression):
    def __init__(self, left, right):
        self.left = left
        self.right = right

    def __eq__(self, other):
        return (
            isinstance(other, SumF)
            and self.left == other.left
            and self.right == other.right
        )

    def __str__(self):
        return f"+({self.left}, {self.right})"

    def instantiate(self, var_mapping, init_facts):
        return self.left.instantiate(var_mapping, init_facts) + self.right.instantiate(
            var_mapping, init_facts
        )


class PrimitiveNumericExpression(FunctionalExpression):
    parts = ()

    def __init__(self, symbol, args):
        self.symbol = symbol
        self.args = tuple(args)
        self.hash = hash((self.__class__, self.symbol, self.args))

    def __hash__(self):
        return self.hash

    def __eq__(self, other):
        return (
            self.__class__ == other.__class__
            and self.symbol == other.symbol
            and self.args == other.args
        )

    def __str__(self):
        return "%s %s(%s)" % ("PNE", self.symbol, ", ".join(map(str, self.args)))

    def dump(self, indent="  "):
        print("%s%s" % (indent, self._dump()))

    def _dump(self):
        return str(self)

    def instantiate(self, var_mapping, init_assignments):
        args = [var_mapping.get(arg, arg) for arg in self.args]
        pne = PrimitiveNumericExpression(self.symbol, args)
        assert self.symbol != "reward"
        # We know this expression is constant. Substitute it by corresponding
        # initialization from task.
        result = init_assignments.get(pne)
        assert result is not None, "Could not find instantiation for PNE: %r" % (
            str(pne),
        )
        return result


class FunctionAssignment(object):
    def __init__(self, fluent, expression):
        self.fluent = fluent
        self.expression = expression

    def __str__(self):
        return "%s %s %s" % (self.__class__.__name__, self.fluent, self.expression)

    def dump(self, indent="  "):
        print("%s%s" % (indent, self._dump()))
        self.fluent.dump(indent + "  ")
        self.expression.dump(indent + "  ")

    def _dump(self):
        return self.__class__.__name__

    def instantiate(self, var_mapping, init_facts):
        expression = self.expression.instantiate(var_mapping, init_facts)
        if not isinstance(expression, NumericConstant):
            raise ValueError("Cannot instantiate assignment: not normalized")
        # We know that this assignment is a cost effect of an action (for initial state
        # assignments, "instantiate" is not called). Hence, we know that the fluent is
        # the 0-ary "reward" which does not need to be instantiated
        assert self.fluent.symbol in [
            "total-cost",
            "reward",
        ], f"unexpected fluent symbol {self.fluent.symbol}"
        fluent = self.fluent
        return FunctionAssignment(fluent, expression)

    @staticmethod
    def MakeSum(f1, f2):
        assert f1.fluent == f2.fluent
        assert not isinstance(f2, Assign) or not isinstance(f1, Assign)
        if isinstance(f2, Assign):
            return FunctionAssignment.AssignSum(f2, f1)
        elif isinstance(f1, Decrease) and isinstance(f2, Decrease):
            return Decrease(f1.fluent, SumF(f1.expression, f2.expression))
        elif isinstance(f2, Decrease):
            return f1.__class__(f1.fluent, SumF(f1.expression, NegateF(f2.expression)))
        elif isinstance(f1, Decrease):
            return f2.__class__(f1.fluent, SumF(f2.expression, NegateF(f1.expression)))
        else:
            return f1.__class__(f1.fluent, SumF(f1.expression, f2.expression))


class Assign(FunctionAssignment):
    def __str__(self):
        return "%s := %s" % (self.fluent, self.expression)


class Increase(FunctionAssignment):
    pass


class Decrease(FunctionAssignment):
    def __init__(self, fluent, expression):
        super().__init__(fluent, NegateF(expression))
